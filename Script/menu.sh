...
# ── Helper: leer proyecto EAS activo ─────────────────────────
_eas_get_project() {
  [ -f "$EAS_PROJECT_FILE" ] && cat "$EAS_PROJECT_FILE" 2>/dev/null || echo ""
}

_eas_set_project() {
  echo "$1" > "$EAS_PROJECT_FILE"
}
...
check_claude() {
  ...
  # Fuente de verdad primaria: wrapper en PREFIX/bin
  # Persiste entre reinicios y pkg installs — más estable que el registry
  ...
  # wrapper existe → Claude Code está instalado aunque el registry esté limpio
  # Reparar registry silenciosamente para que las próximas comprobaciones sean rápidas
...
EOF
  ...
  # cli.js no existe pero wrapper sí → instalación parcial, marcar para reinstalar
  if [ "$cli_ok" = "false" ] && [ "$wrapper_ok" = "true" ]; then
    echo "ready|err:reinstalar|"
    return
  fi
...
check_dashboard() {
  ...
  [ "$(get_reg dashboard installed)" = "true" ] || {
    # Fallback: verificar si los archivos existen aunque el registry no lo diga
    [ -f "$HOME/dashboard/dashboard_server.py" ] || \
  ...
  while true; do
    ...
    case "$OPT" in
      ...
      6)
        ...
        echo ""
        echo -n "  Elige número de modelo a eliminar: "
        read -r CHOICE < /dev/tty
        ...
        if [[ "$CHOICE" =~ ^[0-9]+$ ]] && \
          ...
          SELECTED="${MODELS[$((CHOICE-1))]}"
          echo ""
          echo -n "  ¿Eliminar ${SELECTED}? (s/n): "
          read -r CONFIRMAR < /dev/tty
          if [ "$CONFIRMAR" = "s" ] || [ "$CONFIRMAR" = "S" ]; then
            ollama rm "$SELECTED" 2>/dev/null
            if [ $? -eq 0 ]; then
              echo -e "  ${GREEN}[OK]${NC} Modelo ${SELECTED} eliminado."
            else
              echo -e "  ${RED}[ERROR]${NC} No se pudo eliminar ${SELECTED}."
            fi
          else
    ...
    case "$OPT" in
      ...
      1)
        ...
        if [ ! -f "$CLI_PATH" ]; then
          ...
          echo "  Reinstala Claude Code con [0] → Restore."
          echo ""
          read -r _ < /dev/tty
          continue
        fi
      ...
      2)
        ...
        echo -e "  ${CYAN}Proyectos disponibles en ~/proyectos/:${NC}"
        echo ""
        local IDX=1
        if [ ${#PROJS[@]} -gt 0 ]; then
          for p in "${PROJS[@]}"; do
        ...
        echo "    [d] Usar directorio de Download"
        echo "    [b] Volver"
        echo ""
        echo -n "  Elige opción: "
        ...
        case "$PCHOICE" in
          ...
          d|D)
            ...
            if [[ "$DCHOICE" =~ ^[0-9]+$ ]] && \
              ...
              if [ ! -e "$LINK_DST" ]; then
                ln -s "$LINK_SRC" "$LINK_DST" 2>/dev/null && \
                  echo -e "  ${GREEN}[OK]${NC} Symlink creado: ~/proyectos/${DNAME}" || \
                  echo -e "  ${RED}[ERROR]${NC} No se pudo crear symlink."
                echo ""
              fi
              TARGET_DIR="$LINK_DST"
            else
        ...
        if [ -n "$TARGET_DIR" ]; then
          if [ ! -d "$TARGET_DIR" ]; then
            echo -e "\n  ${RED}[ERROR]${NC} El directorio no existe: ${TARGET_DIR}"
            echo ""
            read -r _ < /dev/tty
            continue
          fi
        ...
        while true; do
          ...
          case "$GOPT" in
            1)
              ...
              echo ""
              mkdir -p "$HOME/proyectos"
              echo -e "  ${CYAN}Proyectos en ~/proyectos/:${NC}"
              echo ""
              mapfile -t LINKS < <(ls -1 "$HOME/proyectos/" 2>/dev/null)
              if [ ${#LINKS[@]} -eq 0 ]; then
                echo "    (ninguno)"
              else
                for lnk in "${LINKS[@]}"; do
                  local FULL="$HOME/proyectos/$lnk"
                  if [ -L "$FULL" ]; then
                    local TARGET
            ...
            2)
              ...
              if [[ "$DCHOICE" =~ ^[0-9]+$ ]] && \
                ...
                local LINK_DST="$HOME/proyectos/${DNAME}"
                if [ -e "$LINK_DST" ]; then
                  echo -e "\n  ${YELLOW}[AVISO]${NC} Ya existe: ~/proyectos/${DNAME}"
                else
                  ln -s "$LINK_SRC" "$LINK_DST" 2>/dev/null && \
                    echo -e "\n  ${GREEN}[OK]${NC} Symlink creado: ~/proyectos/${DNAME}" || \
                    echo -e "\n  ${RED}[ERROR]${NC} No se pudo crear symlink."
                fi
              else
                echo -e "  ${RED}[ERROR]${NC} Número inválido."
              fi
            ...
            3)
              ...
              mapfile -t LINKS < <(ls -1 "$HOME/proyectos/" 2>/dev/null)
              if [ ${#LINKS[@]} -eq 0 ]; then
                echo -e "  ${YELLOW}[AVISO]${NC} No hay proyectos en ~/proyectos/."
                echo ""
                read -r _ < /dev/tty
                continue
              fi
              ...
              echo ""
              echo -n "  Elige número a eliminar: "
              read -r RCHOICE < /dev/tty
              if [[ "$RCHOICE" =~ ^[0-9]+$ ]] && \
                 [ "$RCHOICE" -ge 1 ] && \
                 [ "$RCHOICE" -le "${#LINKS[@]}" ]; then
                local RNAME="${LINKS[$((RCHOICE-1))]}"
                echo ""
                echo -n "  ¿Eliminar symlink '${RNAME}'? (s/n): "
                read -r RCONFIRM < /dev/tty
                if [ "$RCONFIRM" = "s" ] || [ "$RCONFIRM" = "S" ]; then
                  rm -f "$HOME/proyectos/${RNAME}" && \
      ...
      4)
        clear
        echo ""
        echo -e "  ${CYAN}Actualizando Claude Code desde GitHub Releases...${NC}"
        echo ""
        if [ -f "$HOME/restore.sh" ] && [ -s "$HOME/restore.sh" ]; then
          bash "$HOME/restore.sh" --module claude --source github < /dev/tty
        else
...
submenu_expo() {
  while true; do
    ...
    # Leer proyecto activo en cada vuelta
    local ACTIVE_PROJ
    ACTIVE_PROJ=$(_eas_get_project)
    local PROJ_DISPLAY="${ACTIVE_PROJ:-<ninguno>}"
    # Truncar si es muy largo
    [ ${#PROJ_DISPLAY} -gt 32 ] && PROJ_DISPLAY="...${PROJ_DISPLAY: -29}"
    ...
    case "$OPT" in
      ...
      1)
        ...
        echo ""
        local PROJ
        PROJ=$(_eas_get_project)
        if [ -z "$PROJ" ]; then
          echo -e "  ${YELLOW}[AVISO]${NC} Sin proyecto activo."
          echo -n "  Ruta del proyecto (Enter = directorio actual): "
          read -r PROJ < /dev/tty
          [ -z "$PROJ" ] && PROJ="."
        else
          echo -e "  ${CYAN}Proyecto activo:${NC} $PROJ"
          echo -n "  ¿Usar este proyecto? (s/n/ruta): "
          read -r ANS < /dev/tty
      ...
      # ── [6] Configurar proyecto activo ─────────────────────
      6)
        clear
        echo ""
        echo -e "  ${CYAN}${BOLD}Configurar proyecto activo${NC}"
        echo ""
        local CUR
        CUR=$(_eas_get_project)
        if [ -n "$CUR" ]; then
          echo -e "  Actual: ${GREEN}${CUR}${NC}"
          echo ""
        fi

        # Listar proyectos disponibles en ~/proyectos/
        mkdir -p "$HOME/proyectos"
        mapfile -t PROJS < <(ls -1 "$HOME/proyectos/" 2>/dev/null)
        ...
        echo "    [m] Escribir ruta manual"
        echo "    [d] Elegir desde /sdcard/Download/"
        echo "    [x] Limpiar proyecto activo"
        echo "    [b] Cancelar"
        echo ""
        echo -n "  Opción: "
        read -r PCHOICE < /dev/tty
        ...
        case "$PCHOICE" in
          m|M)
            echo ""
            echo -n "  Ruta completa del proyecto: "
          ...
          d|D)
            ...
            mapfile -t DL_DIRS < <(find /storage/emulated/0/Download \
              ...
              | xargs -I{} basename {})
            if [ ${#DL_DIRS[@]} -eq 0 ]; then
              echo -e "  ${YELLOW}[AVISO]${NC} No hay carpetas en Download."
              read -r _ < /dev/tty; continue
            fi
            echo -e "  ${CYAN}Carpetas en Download:${NC}"
            echo ""
            for i in "${!DL_DIRS[@]}"; do
              printf "    [%d] %s\n" "$((i+1))" "${DL_DIRS[$i]}"
            done
            ...
            if [[ "$DCHOICE" =~ ^[0-9]+$ ]] && \
               [ "$DCHOICE" -ge 1 ] && \
               [ "$DCHOICE" -le "${#DL_DIRS[@]}" ]; then
              NEW_PROJ="/storage/emulated/0/Download/${DL_DIRS[$((DCHOICE-1))]}"
            else
              echo -e "  ${RED}[ERROR]${NC} Número inválido."
              read -r _ < /dev/tty; continue
            fi
          ...
          x|X)
            ...
            echo -e "\n  ${GREEN}[OK]${NC} Proyecto activo eliminado."
            read -r _ < /dev/tty; continue
            ;;
          b|B|"")
            continue
        ...
        if [ -n "$NEW_PROJ" ]; then
          # Resolver symlinks para validar
          local REAL_PATH
          REAL_PATH=$(readlink -f "$NEW_PROJ" 2>/dev/null || echo "$NEW_PROJ")
          if [ ! -d "$REAL_PATH" ]; then
            echo -e "\n  ${RED}[ERROR]${NC} Directorio no existe: $NEW_PROJ"
          elif [ ! -f "$REAL_PATH/package.json" ]; then
            echo -e "\n  ${YELLOW}[AVISO]${NC} No encontré package.json en ese directorio."
            ...
            _eas_set_project "$NEW_PROJ"
            echo -e "\n  ${GREEN}[OK]${NC} Proyecto activo: $NEW_PROJ"
          fi
        fi
        echo ""
        read -r _ < /dev/tty
      ...
      7)
        ...
        if [ -z "$PROJ" ]; then
          ...
          echo "  Configúralo con [6] primero."
          echo ""
          read -r _ < /dev/tty
          continue
        fi
        ...
        local REAL_PATH
        REAL_PATH=$(readlink -f "$PROJ" 2>/dev/null || echo "$PROJ")

        if [ ! -d "$REAL_PATH" ]; then
          echo -e "  ${RED}[ERROR]${NC} Directorio no existe: $PROJ"
          echo ""
          read -r _ < /dev/tty
          continue
        fi
        ...
        if [ ! -d "$REAL_PATH/.git" ]; then
          echo -e "  ${RED}[ERROR]${NC} No es un repositorio git: $PROJ"
          echo ""
          echo -n "  ¿Inicializar repositorio git ahora? (s/n): "
          read -r INITGIT < /dev/tty
          if [ "$INITGIT" = "s" ] || [ "$INITGIT" = "S" ]; then
            cd "$REAL_PATH" && git init && \
              echo -e "  ${GREEN}[OK]${NC} Repositorio inicializado." || \
              echo -e "  ${RED}[ERROR]${NC} Falló git init."
          fi
          echo ""
          read -r _ < /dev/tty
          continue
        ...
        cd "$REAL_PATH" || { echo -e "  ${RED}[ERROR]${NC} No se pudo acceder al directorio."; read -r _ < /dev/tty; continue; }

        echo -e "  ${CYAN}Proyecto:${NC} $PROJ"
        echo ""
        echo -e "  ${BOLD}Estado actual:${NC}"
        git status --short
        echo ""
        echo -n "  Mensaje del commit (Enter = 'update desde Android'): "
        ...
        if [ $COMMIT_STATUS -eq 0 ]; then
          ...
          if [ $PUSH_STATUS -eq 0 ]; then
            ...
            echo -e "  ${RED}[ERROR]${NC} Push falló — verifica remote y credenciales."
            echo -e "  ${DIM}Tip: git remote -v  →  ver remotes configurados${NC}"
          fi
        else
          echo ""
...
submenu_python() {
  ...
  while true; do
    ...
    case "$OPT" in
      ...
      # ── [6] Ejecutar script .py ─────────────────────────────
      6)
        clear
        echo ""
        echo -e "  ${CYAN}${BOLD}Ejecutar script Python${NC}"
        echo ""

        # Buscar scripts en ~/proyectos/ y ~/
        ...
        if [ ${#PY_SCRIPTS[@]} -eq 0 ]; then
          echo -e "  ${YELLOW}[AVISO]${NC} No se encontraron scripts .py en:"
          echo "    ~/proyectos/  ~/  /sdcard/Download/"
          echo ""
          echo -n "  Escribe la ruta completa del script: "
          read -r MANUAL_PY < /dev/tty
          if [ -n "$MANUAL_PY" ] && [ -f "$MANUAL_PY" ]; then
          ...
          echo -e "  ${CYAN}Scripts encontrados:${NC}"
          echo ""
          for i in "${!PY_SCRIPTS[@]}"; do
            # Mostrar ruta relativa para legibilidad
            local DISPLAY="${PY_SCRIPTS[$i]}"
            DISPLAY="${DISPLAY/#$HOME/~}"
            DISPLAY="${DISPLAY/#\/storage\/emulated\/0/📱}"
            printf "    [%d] %s\n" "$((i+1))" "$DISPLAY"
          done
          echo ""
          echo "    [m] Escribir ruta manual"
          ...
          local SELECTED_PY="${PY_SCRIPTS[$((SCHOICE-1))]}"
        fi

        local SELECTED_PY="${SELECTED_PY:-${PY_SCRIPTS[0]}}"

        if [ ! -f "$SELECTED_PY" ]; then
          echo -e "  ${RED}[ERROR]${NC} Archivo no existe: $SELECTED_PY"
          read -r _ < /dev/tty
          continue
        fi

        echo ""
        echo -e "  ${CYAN}▶ Ejecutando:${NC} $SELECTED_PY"
        echo -e "  ${DIM}─────────────────────────────────────────${NC}"
        echo ""

        # Ejecutar en el directorio del script
        local SCRIPT_DIR
        SCRIPT_DIR=$(dirname "$SELECTED_PY")
        ...
        if [ $PY_EXIT -eq 0 ]; then
          echo -e "  ${GREEN}[OK]${NC} Script terminó (código 0)"
        else
          echo -e "  ${YELLOW}[AVISO]${NC} Script terminó con código $PY_EXIT"
        fi
        echo ""
        read -r _ < /dev/tty
        ;;
  ...
  case "$OPT" in
    ...
    7)
      if [ "$DB_STATE" = "not_installed" ]; then
        ...
        echo ""
        echo -e "${CYAN}${BOLD}  ╔══════════════════════════════════════════╗"
        echo    "  ║  ⬡ Dashboard web — Instalar             ║"
        echo    "  ╠══════════════════════════════════════════╣"
        echo -e "  ║  ${NC}Descarga los archivos del dashboard      ${CYAN}${BOLD}║"
        echo -e "  ║  ${NC}desde el repositorio GitHub.             ${CYAN}${BOLD}║"
        echo    "  ║                                          ║"
        echo -e "  ║  ${NC}También instala:                         ${CYAN}${BOLD}║"
        echo -e "  ║  ${NC}  · termux-api (pkg)                     ${CYAN}${BOLD}║"
        echo -e "  ║  ${NC}    Necesario para la app Android        ${CYAN}${BOLD}║"
        echo    "  ║                                          ║"
        echo -e "  ║  ${NC}[1] Instalar dashboard + termux-api      ${CYAN}${BOLD}║"
        echo -e "  ║  ${NC}[b] Cancelar${CYAN}${BOLD}                             ║"
        echo -e "  ╚══════════════════════════════════════════╝${NC}"
        echo ""
        read -r -p "  Opción: " DB_OPT < /dev/tty

        if [ "$DB_OPT" = "1" ]; then
          clear
          echo ""
          echo -e "${CYAN}${BOLD}  Instalando Dashboard...${NC}"
          echo ""

          DASHBOARD_RAW="https://raw.githubusercontent.com/Honkonx/termux-ai-stack/main/Script/dashboard"
          DASHBOARD_DIR="$HOME/dashboard"
          INSTALL_OK=true

          # ── Crear directorio dashboard ──────────────────────
          mkdir -p "$DASHBOARD_DIR"

          # ── Descargar archivos del dashboard ───────────────
          DB_FILES=("dashboard_server.py" "dashboard_start.sh" "dashboard_stop.sh" "index.html")
          for F in "${DB_FILES[@]}"; do
            echo -n "  Descargando $F... "
            curl -fsSL "$DASHBOARD_RAW/$F" -o "$DASHBOARD_DIR/$F" 2>/dev/null || \
              wget -q "$DASHBOARD_RAW/$F" -O "$DASHBOARD_DIR/$F" 2>/dev/null
            if [ -f "$DASHBOARD_DIR/$F" ] && [ -s "$DASHBOARD_DIR/$F" ]; then
              chmod +x "$DASHBOARD_DIR/$F"
              echo -e "${GREEN}✓${NC}"
            else
              echo -e "${RED}✗ error${NC}"
              INSTALL_OK=false
          ...
          # ── Crear scripts de control en HOME (compat. con menú) ──
          echo -n "  Creando dashboard_start.sh... "
          cat > "$HOME/dashboard_start.sh" << 'DBSTART'
#!/data/data/com.termux/files/usr/bin/bash
DASHBOARD_DIR="$HOME/dashboard"
if [ ! -f "$DASHBOARD_DIR/dashboard_server.py" ]; then
  echo "[ERROR] dashboard_server.py no encontrado en $DASHBOARD_DIR"
  exit 1
fi
pgrep -f "dashboard_server.py" &>/dev/null && {
  echo "[INFO] Dashboard ya está corriendo en :8080"
  exit 0
}
cd "$DASHBOARD_DIR"
...
DBSTOP
          ...
          # ── Instalar termux-api (necesario para app Android) ─
          echo ""
          echo -e "  ${CYAN}[INFO]${NC} Instalando termux-api..."
          pkg install termux-api -y \
            -o Dpkg::Options::="--force-confdef" \
            -o Dpkg::Options::="--force-confold" 2>/dev/null && \
            echo -e "  ${GREEN}[OK]${NC} termux-api instalado" || \
            echo -e "  ${YELLOW}[AVISO]${NC} termux-api: error (puede ya estar instalado)"

          # ── Registrar en registry ───────────────────────────
          if [ "$INSTALL_OK" = "true" ]; then
            grep -v "^dashboard\." "$REGISTRY" > "$REGISTRY.tmp" 2>/dev/null || touch "$REGISTRY.tmp"
            cat >> "$REGISTRY.tmp" << EOF
dashboard.installed=true
dashboard.version=1.1
dashboard.install_date=$(date +%Y-%m-%d)
dashboard.port=8080
...
EOF
            ...
            echo ""
            echo -e "  ${GREEN}${BOLD}╔══════════════════════════════════════════╗"
            echo    "  ║  ✓ Dashboard instalado con éxito       ║"
            echo -e "  ╚══════════════════════════════════════════╝${NC}"
            echo ""
            echo -e "  ${CYAN}IMPORTANTE — app Android:${NC}"
            echo "  Para que la app pueda controlar el stack"
            echo "  necesitás instalar Termux:API desde F-Droid:"
            echo ""
            echo -e "  ${YELLOW}  https://f-droid.org/packages/com.termux.api/${NC}"
            echo ""
            echo "  (el paquete termux-api ya está instalado ✓)"
          else
            echo ""
            echo -e "  ${RED}[ERROR]${NC} Algunos archivos no se descargaron."
            echo "  Verifica tu conexión a internet."
          fi
          echo ""
          read -r _ < /dev/tty
