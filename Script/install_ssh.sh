...
get_local_ip() {
  local ip
  # Método 1: ip route — más confiable, funciona con wlan0, wlan1, rmnet, etc.
  ip=$(ip route get 1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src"){print $(i+1); exit}}')
  # Método 2: ip addr — fallback
...
# Detectar IP local (funciona con wlan0, wlan1, rmnet, etc.)
_get_ip() {
  local ip
  ip=$(ip route get 1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src"){print $(i+1); exit}}')
  [ -z "$ip" ] && ip=$(ip addr show 2>/dev/null | grep "inet " | grep -v "127\." | awk '{print $2}' | cut -d'/' -f1 | head -1)
...
alias ssh-info='IP=$(ip route get 1 2>/dev/null | awk "{for(i=1;i<=NF;i++) if(\$i==\"src\"){print \$(i+1); exit}}"); [ -z "$IP" ] && IP=$(ip addr show 2>/dev/null | grep "inet " | grep -v "127\." | awk "{print \$2}" | cut -d"/" -f1 | head -1); echo "Conectar: ssh -p 8022 $(whoami)@${IP:-<tu_IP_WiFi>}"'
