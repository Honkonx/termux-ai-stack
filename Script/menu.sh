...
check_remote() {
  ...
  # Detección estricta: no basta con que el binario sshd exista
  # (openssh se instala como dependencia base y no significa que esté configurado)
  # SSH configurado = registry "true" O sshd_config personalizado existe
  local has_ssh=false has_dashboard=false
  local sshd_config="$TERMUX_PREFIX/etc/ssh/sshd_config"
  if [ "$ssh_installed" = "true" ]; then
    has_ssh=true
  elif command -v sshd &>/dev/null && [ -f "$sshd_config" ]; then
    # Solo si existe sshd_config Y tiene el puerto 8022 configurado por nosotros
    grep -q "Port 8022" "$sshd_config" 2>/dev/null && has_ssh=true
  fi

  # Dashboard configurado = registry "true" O dashboard_server.py existe
  if [ "$dashboard_installed" = "true" ]; then
    has_dashboard=true
  elif [ -f "$HOME/dashboard_server.py" ] || [ -f "$HOME/dashboard/dashboard_server.py" ]; then
    has_dashboard=true
  fi
