...
while true; do
  ...
  case "$OPT" in
    ...
    p|P)
      clear; echo ""
      echo -e "${CYAN}${BOLD}  ╔══════════════════════════════════════════╗"
      echo    "  ║  ⚡ Rendimiento Termux                   ║"
      echo    "  ╠══════════════════════════════════════════╣"
      echo -e "  ║  ${NC}Paquetes que mejoran velocidad y uso de  ${CYAN}${BOLD}║"
      echo -e "  ║  ${NC}recursos en Termux (sin root)            ${CYAN}${BOLD}║"
      echo    "  ╠══════════════════════════════════════════╣"
      echo -e "  ║  ${NC}termux-wake-lock  → CPU sin throttling   ${CYAN}${BOLD}║"
      echo -e "  ║  ${NC}htop              → monitor de procesos  ${CYAN}${BOLD}║"
      echo -e "  ║  ${NC}nload             → monitor de red       ${CYAN}${BOLD}║"
      echo -e "  ║  ${NC}zstd              → compresión rápida    ${CYAN}${BOLD}║"
      echo -e "  ║  ${NC}binutils          → herramientas binarias${CYAN}${BOLD}║"
      echo -e "  ║  ${NC}clang             → compilador ARM64     ${CYAN}${BOLD}║"
      echo    "  ╠══════════════════════════════════════════╣"
      echo -e "  ║  ${YELLOW}NOTA: GPU y CPU boost requieren root    ${CYAN}${BOLD}║"
      echo -e "  ║  ${YELLOW}termux-wake-lock evita throttling       ${CYAN}${BOLD}║"
      echo -e "  ╚══════════════════════════════════════════╝${NC}"
      echo ""
      echo -n "  ¿Instalar todos? (s/n): "
      read -r PERF_CONFIRM < /dev/tty
      if [ "$PERF_CONFIRM" = "s" ] || [ "$PERF_CONFIRM" = "S" ]; then
        echo ""
        info() { echo -e "  ${CYAN}[INFO]${NC} $1"; }
        log()  { echo -e "  ${GREEN}[OK]${NC} $1"; }
        info "Instalando paquetes de rendimiento..."
        pkg install -y           -o Dpkg::Options::="--force-confdef"           -o Dpkg::Options::="--force-confold"           htop nload zstd binutils clang termux-api 2>/dev/null ||           echo -e "  ${YELLOW}[AVISO]${NC} Algunos paquetes tuvieron advertencias"
        echo ""
        log "Paquetes instalados"
        echo ""
        echo -e "  ${CYAN}termux-wake-lock${NC} — para activar manualmente:"
        echo "  Ejecuta: termux-wake-lock"
        echo "  (requiere Termux:API app desde F-Droid)"
        echo ""
        echo -e "  ${CYAN}Para activar wake-lock al inicio de servicios pesados:${NC}"
        echo "  Agrega 'termux-wake-lock' al inicio de ollama_start.sh o n8n start"
      fi
      echo ""; read -r _ < /dev/tty ;;
    s|S|q|Q|"")
      clear; echo ""
