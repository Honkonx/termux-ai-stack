...
# Validar instalación existente — cli.js debe existir Y ser JS válido (no wrapper bash)
_CLI_VALID=false
if [ -f "$CLI_PATH" ] && [ -s "$CLI_PATH" ]; then
  if node "$CLI_PATH" --version 2>&1 | grep -qv "SyntaxError"; then
    _CLI_VALID=true
  fi
fi

if [ "$_CLI_VALID" = "true" ]; then
  ...
  warn "cli.js existe pero está corrompido (wrapper bash de versión latest)"
  warn "Forzando reinstalación limpia con v2.1.111..."
  rm -f "$CHECKPOINT"
fi
...
# Checkpoint con validación — aunque diga hecho, verificar cli.js
_CLI_CHECK=$(find_cli)
_SKIP_INSTALL=false
if check_done "claude_npm" && [ -f "$_CLI_CHECK" ] && [ -s "$_CLI_CHECK" ]; then
  if node "$_CLI_CHECK" --version 2>&1 | grep -qv "SyntaxError"; then
    log "Claude Code ya instalado y válido [checkpoint]"
    _SKIP_INSTALL=true
  else
    warn "Checkpoint inválido — cli.js corrompido, reinstalando..."
...
if [ "$_SKIP_INSTALL" = "false" ]; then
  ...
  # ── Estrategia 1: npm install @2.1.111 ───────────────────────
  # npm instala la versión correcta pero el script post-install
  # puede sobreescribir cli.js con un wrapper bash en algunos entornos.
  # Por eso validamos SIEMPRE con node después de instalar.
  info "Instalando v${CLAUDE_VERSION_FALLBACK} vía npm..."
  npm install -g @anthropic-ai/claude-code@${CLAUDE_VERSION_FALLBACK} --ignore-scripts 2>&1 | tail -5
  echo ""
  ...
  if [ -f "$CLI_PATH_NEW" ] && [ -s "$CLI_PATH_NEW" ]; then
    if node "$CLI_PATH_NEW" --version 2>&1 | grep -qv "SyntaxError"; then
      CLAUDE_OK=true
      log "v${CLAUDE_VERSION_FALLBACK} instalada y validada vía npm ✓"
    fi
  fi

  # ── Estrategia 2: descargar cli.js directamente desde GitHub Releases ──
  # Si npm instaló el paquete correcto pero cli.js quedó corrompido,
  # descargamos el cli.js original del release y lo reemplazamos.
  if [ "$CLAUDE_OK" = "false" ] && [ -d "$CLAUDE_DIR" ]; then
    warn "npm instaló el directorio pero cli.js es inválido — reparando desde GitHub..."
    RELEASE_API="https://api.github.com/repos/Honkonx/termux-ai-stack/releases/latest"
    CLI_URL=$(curl -fsSL "$RELEASE_API" 2>/dev/null |       grep -o '"browser_download_url": *"[^"]*part2-claude[^"]*"' |       grep -o 'https://[^"]*' | head -1)

    if [ -n "$CLI_URL" ]; then
      info "Descargando part2-claude-code desde GitHub Releases..."
      TMP_TAR="$HOME/claude_tmp.tar.xz"
      curl -fL --progress-bar "$CLI_URL" -o "$TMP_TAR" 2>/dev/null
      if [ -f "$TMP_TAR" ] && [ -s "$TMP_TAR" ]; then
        TMP_EXTRACT="$HOME/claude_extract_tmp"
        mkdir -p "$TMP_EXTRACT"
        tar -xJf "$TMP_TAR" -C "$TMP_EXTRACT" 2>/dev/null
        if [ -f "$TMP_EXTRACT/npm_modules/@anthropic-ai/claude-code/cli.js" ]; then
          cp "$TMP_EXTRACT/npm_modules/@anthropic-ai/claude-code/cli.js" "$CLAUDE_DIR/cli.js"
          chmod +x "$CLAUDE_DIR/cli.js"
          if node "$CLI_PATH_NEW" --version 2>&1 | grep -qv "SyntaxError"; then
            CLAUDE_OK=true
            log "cli.js reparado desde GitHub Releases ✓"
          fi
        fi
        rm -rf "$TMP_EXTRACT" "$TMP_TAR"
      fi
    else
      warn "No se pudo obtener URL del release de GitHub"
    fi
  fi

  # ── Estrategia 3: instalar directamente desde el tar del release ──
  if [ "$CLAUDE_OK" = "false" ]; then
    warn "Instalando directamente desde GitHub Releases (sin npm)..."
    RELEASE_API="https://api.github.com/repos/Honkonx/termux-ai-stack/releases/latest"
    CLI_URL=$(curl -fsSL "$RELEASE_API" 2>/dev/null |       grep -o '"browser_download_url": *"[^"]*part2-claude[^"]*"' |       grep -o 'https://[^"]*' | head -1)
    ...
    if [ -n "$CLI_URL" ]; then
      ...
      if [ -f "$TMP_TAR" ] && [ -s "$TMP_TAR" ]; then
        ...
        mkdir -p "$TMP_EXTRACT"
        tar -xJf "$TMP_TAR" -C "$TMP_EXTRACT" 2>/dev/null
        if [ -d "$TMP_EXTRACT/npm_modules/@anthropic-ai" ]; then
          mkdir -p "$NPM_ROOT"
          rm -rf "$NPM_ROOT/@anthropic-ai" 2>/dev/null
          cp -r "$TMP_EXTRACT/npm_modules/@anthropic-ai" "$NPM_ROOT/"
          CLI_PATH_NEW=$(find_cli)
          if [ -f "$CLI_PATH_NEW" ] && node "$CLI_PATH_NEW" --version 2>&1 | grep -qv "SyntaxError"; then
            CLAUDE_OK=true
            log "Claude Code instalado desde GitHub Releases ✓"
          fi
        fi
        rm -rf "$TMP_EXTRACT" "$TMP_TAR"
      fi
  ...
  if [ "$CLAUDE_OK" = "false" ]; then
    echo ""
    echo -e "${RED}[ERROR]${NC} Claude Code no instaló correctamente en ninguna estrategia"
    echo ""
    echo "  Diagnóstico:"
    ...
    echo ""
    echo "  Prueba manual (desde tu backup):"
    echo "  cp -r npm_modules/@anthropic-ai $NPM_ROOT/"
    exit 1
  fi
