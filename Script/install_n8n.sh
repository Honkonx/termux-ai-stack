...
case "$INSTALL_MODE" in
  1)
    ...
    info "Opción 1 — Todo desde GitHub Releases..."
    ensure_restore_sh
  ...
  2)
    ...
    info "Opción 2 — Instalación completamente limpia..."
    CLEAN_ROOTFS=true
    CLEAN_N8N=true
    ;;
  3)
    # Rootfs GitHub + n8n limpio (npm)
    info "Opción 3 — Rootfs desde GitHub + n8n instalación limpia..."
    ensure_restore_sh
    bash "$HOME/restore.sh" --module proot --source github
    CLEAN_ROOTFS=false
    CLEAN_N8N=true
  ...
  4)
    ...
    info "Opción 4 — Rootfs limpio + n8n desde GitHub..."
    ensure_restore_sh
    CLEAN_ROOTFS=true
    CLEAN_N8N=false
    ;;
  q|Q)
    ...
    ;;
  *)
    warn "Opción inválida — usando opción 2 (todo limpio)"
    CLEAN_ROOTFS=true
    CLEAN_N8N=true
    ;;
esac
