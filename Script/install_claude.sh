...
if validate_cli "$CLI_PATH"; then
  ...
  echo ""
  read -r -p "  ¿Reinstalar de todas formas? (s/n): " REINSTALL < /dev/tty
...
if [ "$_SKIP_INSTALL" = "false" ]; then
  ...
  npm install -g @anthropic-ai/claude-code@${CLAUDE_VERSION} 2>&1 | tail -8
  echo ""
...
alias claude-update='npm uninstall -g @anthropic-ai/claude-code 2>/dev/null; npm cache clean --force; npm install -g @anthropic-ai/claude-code@2.1.111 && echo "Claude Code actualizado"'
ALIASES
