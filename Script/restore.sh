...
show_header() {
  ...
  cat << 'HEADER'
  ╔══════════════════════════════════════════════════╗
  ║   termux-ai-stack · Restore v2.4.0              ║
...
restore_part1() {
  ...
  # Leer versión de menu.sh restaurado
  local BASE_VER
  BASE_VER=$(grep -oE "VERSIÓN: [0-9]+\.[0-9]+\.[0-9]+" "$HOME/menu.sh" 2>/dev/null | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -1)
  [ -z "$BASE_VER" ] && BASE_VER=$(grep -oE "VERSION: [0-9]+\.[0-9]+\.[0-9]+" "$HOME/menu.sh" 2>/dev/null | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -1)
  [ -z "$BASE_VER" ] && BASE_VER="restored"
  update_registry "base" "$BASE_VER"
...
restore_part3() {
  ...
  # Leer versión real de eas-cli
  local EXPO_VER
  EXPO_VER=$(node "$NPM_GLOBAL/eas-cli/bin/eas" --version 2>/dev/null | head -1 | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -1)
  [ -z "$EXPO_VER" ] && EXPO_VER=$(cat "$NPM_GLOBAL/eas-cli/package.json" 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('version',''))" 2>/dev/null)
  [ -z "$EXPO_VER" ] && EXPO_VER="restored"
  update_registry "expo" "$EXPO_VER"
...
restore_part5() {
  ...
  if [ -z "$DISTRO_NAME" ]; then
    ...
    echo ""
    echo -n "  ¿Descargar e instalar el rootfs Debian (part6) ahora? (s/n): "
    read -r DO_PROOT
    echo ""
  ...
  # Verificar que el archivo no esta vacio/corrupto
  local FILE_SIZE
  FILE_SIZE=$(wc -c < "$DOWNLOADED_FILE" 2>/dev/null)
  if [ -z "$FILE_SIZE" ] || [ "$FILE_SIZE" -lt 1024 ]; then
    error "Archivo descargado corrupto (tamano: ${FILE_SIZE:-0} bytes)"
  fi
  info "Archivo verificado: $(du -h "$DOWNLOADED_FILE" | cut -f1)"

  # Copiar al /tmp del rootfs
  local ROOTFS_TMP="${ROOTFS_PATH}tmp"
  mkdir -p "$ROOTFS_TMP"
  cp "$DOWNLOADED_FILE" "$ROOTFS_TMP/n8n_restore.tar.xz"

  if [ ! -s "$ROOTFS_TMP/n8n_restore.tar.xz" ]; then
    error "No se pudo copiar el archivo al proot ($ROOTFS_TMP)"
  fi
  log "Archivo copiado al proot OK"
...
if [ ! -f "$ARCHIVE" ]; then
  echo "[ERROR] No se encontro /tmp/n8n_restore.tar.xz dentro del proot"
  exit 1
fi

FSIZE=$(wc -c < "$ARCHIVE" 2>/dev/null)
echo "[INFO] Tamano dentro del proot: ${FSIZE} bytes"
...
tar -xJf "$ARCHIVE" -C / && echo "[OK] Extraccion completada" || {
  echo "[AVISO] tar reporto errores, intentando con --ignore-failed-read..."
  tar -xJf "$ARCHIVE" -C / --ignore-failed-read &&     echo "[OK] Extraccion completada con advertencias" || {
    echo "[ERROR] Extraccion fallida definitivamente"
    exit 1
  }
}
...
if [ -f /usr/local/bin/n8n ]; then
  echo "[OK] n8n verificado en /usr/local/bin/n8n"
else
  echo "[AVISO] n8n no encontrado en /usr/local/bin"
fi
...
PROOT_INNER
  ...
  if [ $? -ne 0 ]; then
    error "Fallo la restauracion de n8n dentro del proot"
  fi

  # Leer versión de n8n desde el proot
  local N8N_VER
  N8N_VER=$(proot-distro login "$DISTRO_NAME" -- node /usr/local/lib/node_modules/n8n/package.json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('version',''))" 2>/dev/null)
  [ -z "$N8N_VER" ] && N8N_VER=$(proot-distro login "$DISTRO_NAME" -- bash -c "cat /usr/local/lib/node_modules/n8n/package.json 2>/dev/null" | python3 -c "import sys,json; print(json.load(sys.stdin).get('version',''))" 2>/dev/null)
  [ -z "$N8N_VER" ] && N8N_VER="restored"
  update_registry "n8n" "$N8N_VER"
  log "n8n + cloudflared restaurado OK"
}
...
restore_part6() {
  ...
  # Leer versión de proot-distro
  local PROOT_VER
  PROOT_VER=$(proot-distro login "$DISTRO_IN_TAR" -- bash -c "cat /etc/debian_version 2>/dev/null" 2>/dev/null | tr -d '\n')
