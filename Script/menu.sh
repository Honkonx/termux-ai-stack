...
EOF
  ...
  [ -z "$ver" ] && ver=$(node "$cli_path" --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
  echo "ready|${ver}|"
}
...
while true; do
  ...
  # CPU % — snapshot /proc/stat en ~100ms (sin top, sin root)
  CPU_USAGE="--"
  if [ -r /proc/stat ]; then
    read -r _ c1_user c1_nice c1_sys c1_idle c1_iowait c1_irq c1_sirq _ < /proc/stat
    sleep 0.1
    read -r _ c2_user c2_nice c2_sys c2_idle c2_iowait c2_irq c2_sirq _ < /proc/stat
    IDLE_DELTA=$(( (c2_idle + c2_iowait) - (c1_idle + c1_iowait) ))
    TOTAL_DELTA=$(( (c2_user + c2_nice + c2_sys + c2_idle + c2_iowait + c2_irq + c2_sirq) \
                  - (c1_user + c1_nice + c1_sys + c1_idle + c1_iowait + c1_irq + c1_sirq) ))
    if [ "$TOTAL_DELTA" -gt 0 ]; then
      CPU_USAGE=$(( (TOTAL_DELTA - IDLE_DELTA) * 100 / TOTAL_DELTA ))
    fi
  fi

  # Temperatura — termux-battery-status si disponible, fallback --
  CPU_TEMP="--"
  if command -v termux-battery-status &>/dev/null; then
    _TEMP_RAW=$(termux-battery-status 2>/dev/null | \
      python3 -c "import sys,json; d=json.load(sys.stdin); print(int(d.get('temperature',0)))" 2>/dev/null)
    [ -n "$_TEMP_RAW" ] && [ "$_TEMP_RAW" -gt 0 ] 2>/dev/null && CPU_TEMP="${_TEMP_RAW}"
  fi

  # Almacenamiento libre en /data (Termux vive aquí)
  ...
  TEMP_COL="${GREEN}"
  if [ "$CPU_TEMP" != "--" ]; then
    if [ "$CPU_TEMP" -gt 45 ] 2>/dev/null; then
      TEMP_COL="$RED"
    elif [ "$CPU_TEMP" -gt 38 ] 2>/dev/null; then
      TEMP_COL="$YELLOW"
    fi
  fi

  # ── Construir línea RAM/CPU/Temp con padding fijo (44 chars visibles) ──
  # Los colores ANSI no tienen longitud visible — padding manual obligatorio
  _RAM_LBL="RAM: ${RAM_FREE}"
  [ "$CPU_USAGE" = "--" ] && _CPU_LBL="CPU: --" || _CPU_LBL="CPU: ${CPU_USAGE}%"
  [ "$CPU_TEMP"  = "--" ] && _TMP_LBL="Temp: --" || _TMP_LBL="Temp: ${CPU_TEMP}°C"
  _PLAIN_LINE="${_RAM_LBL}  ${_CPU_LBL}  ${_TMP_LBL}"
  _VISIBLE_LEN=${#_PLAIN_LINE}
  _PAD=$(( 40 - _VISIBLE_LEN ))
  [ "$_PAD" -lt 0 ] && _PAD=0
  _SPACES=$(printf '%*s' "$_PAD" '')

  # Valor para echo (colores solo si hay dato real)
  [ "$CPU_USAGE" = "--" ] && _CPU_VAL="--" || _CPU_VAL="${CPU_COL}${CPU_USAGE}%${NC}${CYAN}${BOLD}"
  [ "$CPU_TEMP"  = "--" ] && _TMP_VAL="--" || _TMP_VAL="${TEMP_COL}${CPU_TEMP}°C${NC}${CYAN}${BOLD}"

  # ── Header ───────────────────────────────────────────────────
  echo -e "${CYAN}${BOLD}"
  echo    "  ╔══════════════════════════════════════════╗"
  printf  "  ║  %-40s║\n" "⬡ TERMUX·AI·STACK"
  echo -e "  ║  RAM: ${GREEN}${RAM_FREE}${NC}${CYAN}${BOLD}  CPU: ${_CPU_VAL}  Temp: ${_TMP_VAL}${_SPACES}║"
  printf  "  ║  %-40s║\n" \
    "$([ -n "$IP" ] && echo "IP: $IP  Disk: $DISK_FREE libre" || echo "Disk: $DISK_FREE libre")"
  echo    "  ╠══════════════════════════════════════════╣"
