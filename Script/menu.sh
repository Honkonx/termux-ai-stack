...
while true; do
  ...
  # Temperatura — solo si termux-api disponible, nunca bloquea
  CPU_TEMP="--"
  TEMP_COL="${GREEN}"
  if command -v termux-battery-status &>/dev/null; then
    ...
    if [ -n "$_TEMP_RAW" ] && [ "$_TEMP_RAW" -gt 0 ] 2>/dev/null; then
      CPU_TEMP="$_TEMP_RAW"
      [ "$CPU_TEMP" -gt 45 ] 2>/dev/null && TEMP_COL="$RED"
      [ "$CPU_TEMP" -le 45 ] && [ "$CPU_TEMP" -gt 38 ] 2>/dev/null && TEMP_COL="$YELLOW"
    fi
  fi
  ...
  # Padding manual para linea con colores ANSI (longitud invisible)
  [ "$CPU_TEMP" = "--" ] && _TMP_LBL="Temp: --" || _TMP_LBL="Temp: ${CPU_TEMP}C"
  _LINE2_PLAIN="RAM: ${RAM_FREE}  ${_TMP_LBL}"
  _PAD2=$(( 40 - ${#_LINE2_PLAIN} ))
  [ "$_PAD2" -lt 0 ] && _PAD2=0
  _SP2=$(printf '%*s' "$_PAD2" '')
  [ -n "$IP" ] && _LINE3="IP: $IP  Disk: $DISK_FREE" || _LINE3="Disk: $DISK_FREE libre"

  # ── Header ───────────────────────────────────────────────────
  echo -e "${CYAN}${BOLD}"
  echo    "  ╔══════════════════════════════════════════╗"
  printf  "  ║  %-40s║\n" "⬡ TERMUX·AI·STACK"
  echo -e "  ║  RAM: ${GREEN}${RAM_FREE}${NC}${CYAN}${BOLD}  Temp: ${TEMP_COL}${CPU_TEMP}°C${NC}${CYAN}${BOLD}${_SP2}║"
  printf  "  ║  %-40s║\n" "$_LINE3"
  echo    "  ╠══════════════════════════════════════════╣"
