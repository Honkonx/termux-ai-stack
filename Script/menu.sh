  ...
  case "$OPT" in
    ...
    2)
      if [ "$CC_STATE" = "not_installed" ]; then
        ...
        echo ""
        echo -e "  ${RED}╔══════════════════════════════════════════╗${NC}"
        echo -e "  ${RED}║  ⚠  cli.js corrompido                   ║${NC}"
        echo -e "  ${RED}╚══════════════════════════════════════════╝${NC}"
        echo ""
        echo "  El cli.js de Claude Code es un wrapper bash"
        echo "  incompatible con Termux ARM64."
        echo ""
        echo -n "  ¿Reinstalar Claude Code ahora? (s/n): "
        read -r _ANS < /dev/tty
        if [ "$_ANS" = "s" ] || [ "$_ANS" = "S" ]; then
          # Limpiar todo antes de reinstalar
          npm uninstall -g @anthropic-ai/claude-code 2>/dev/null || true
