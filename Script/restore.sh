...
#    bash ~/restore.sh --module all       → todos los módulos
#
#  MÓDULOS DISPONIBLES:
#    base | claude | expo | ollama | n8n | proot | all
#
#  FUENTES:
#    [1] GitHub Releases — último release automático
#    [2] Backup local   — /sdcard/Download/termux-ai-stack-releases/
#        (solo disponible si el usuario generó backup con backup.sh)
#
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --module)
        ...
        ;;
      *)
        error "Argumento desconocido: $1\n  USO: bash ~/restore.sh [--module <módulo>]"
        ;;
    esac
    shift
  done
...
show_header() {
  ...
  cat << 'HEADER'
  ╔══════════════════════════════════════════════════╗
  ║   termux-ai-stack · Restore v2.0.0              ║
...
restore_part5() {
  ...
  # Verificar que existe el proot
  if [ -z "$DISTRO_NAME" ]; then
    error "No se encontró proot Debian instalado\n  Restaura primero part6 (proot) o instala proot-distro"
  fi
...
if [ -z "$TARGET_MODULE" ]; then
  # Sin args → menú interactivo también elige fuente
  show_header
  select_source
  menu_interactivo
else
  # Con --module → mostrar cabecera y elegir fuente
  show_header
  select_source
fi
