...
parse_args() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --module)
        ...
        ;;
      *)
        error "Argumento desconocido: $1\n  USO: bash ~/backup.sh [--module <módulo>]"
        ;;
    esac
    shift
  done
  ...
  if [ -n "$TARGET_MODULE" ]; then
    case "$TARGET_MODULE" in
      base|claude|expo|ollama|n8n) ;;
      *)
        error "Módulo inválido: '$TARGET_MODULE'\n  Válidos: base | claude | expo | ollama | n8n"
        ;;
    esac
  fi
}
...
if ! $HAS_PROOT; then
  ...
  echo ""
  read -r -p "  ¿Crear part6 ahora? (s/n): " DO_PART6
