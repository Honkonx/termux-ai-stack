...
# ── Detección de estado ───────────────────────────────────────
# Formato de salida: "estado|version|extra"
...
# ── Dibujar módulo ────────────────────────────────────────────
# $1=num $2=icono $3=nombre $4=estado $5=version $6=comando_principal
draw_module() {
  ...
  # Línea 1: número · icono · nombre · estado · comando
...
install_module() {
  ...
  echo ""
  read -r -p "  Presiona ENTER para volver al menú..." _
  # El loop re-leerá el registry automáticamente al volver
}
...
module_action() {
  ...
  case "$state" in
    ...
    running)
      case "$num" in
        ...
        3) tmux attach -t "ollama-server" 2>/dev/null || \
    ...
    stopped|ready)
      case "$num" in
        1) clear && n8n-start 2>/dev/null
           read -r -p "  Presiona ENTER para volver al menú..." _ ;;
        2) clear && claude ;;
        3) clear && ollama-start 2>/dev/null
           read -r -p "  Presiona ENTER para volver al menú..." _ ;;
  ...
  # Al salir de module_action el loop vuelve al inicio y re-lee el registry
}
...
while true; do
  ...
  # ── Re-leer estado en cada iteración del loop ─────────────────
  # (se ejecuta al inicio, tras instalar, tras cualquier acción)
  ...
  # ── Módulos con comando visible a la derecha ──────────────────
  # Comando depende del estado: running→logs, stopped→iniciar, ready→usar
  ...
  # Ollama
  case "$OL_STATE" in
    running) OL_CMD="ollama-stop" ;;
    stopped) OL_CMD="ollama-start" ;;
    *)       OL_CMD="" ;;
  esac
  ...
  case "$OPT" in
    ...
    # Alias directos escritos desde el prompt del menú
