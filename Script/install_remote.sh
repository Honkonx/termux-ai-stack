...
if check_done "dashboard_install"; then
  ...
  # SIEMPRE sobrescribir dashboard_start.sh con versión robusta (sin tmux)
  # El script del repo puede depender de tmux que no siempre está disponible
  cat > "$HOME/dashboard_start.sh" << 'DBSTART_ROBUST'
#!/data/data/com.termux/files/usr/bin/bash
# dashboard_start.sh — versión robusta sin dependencia de tmux
...
if pgrep -f "dashboard_server.py" &>/dev/null; then
  IP=$(_get_ip)
  echo "[INFO] Dashboard ya corriendo en http://${IP}:8080"
  exit 0
fi
