// src/screens/modules/claude/ClaudeScreen.js — v3.0.0 (S16)
// - Sección API Key/Endpoint removida (auth via OAuth en terminal)
// - Proyectos: lista + crear symlink + eliminar
// - Botón "Abrir Claude Code" → TerminalModal
// - Info técnica limpia
...
export function ClaudeScreen({ navigate, goBack }) {
  ...
  const [termTitle, setTermTitle]         = useState('Claude Code');
  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      ...
      setProjects(list);
      if (selectedProj && !list.find(p => p.name === selectedProj.name)) {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Eliminar', style: 'destructive', onPress: async () => {
          try {
            await apiFetch('/api/claude/project/delete', {
              method: 'POST',
  ...
  const handleOpenTerminal = useCallback(() => {
    const proj = selectedProj?.name;
    setTermTitle(proj ? `claude · ${proj}` : 'Claude Code');
    setTermVisible(true);
  }, [selectedProj]);
    ...
    <View style={s.root}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent}
        ...
        <TouchableOpacity onPress={handleOpenTerminal} style={s.termBtn} activeOpacity={0.8}>
          <View style={s.termBtnIcon}>
            <Text style={s.termBtnGlyph}>&gt;_</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.termBtnLabel}>Abrir Claude Code</Text>
            <Text style={s.termBtnSub} numberOfLines={1}>
              {selectedProj
                ? `proyecto: ${selectedProj.name}`
                : 'sin proyecto seleccionado · directorio ~/'}
            </Text>
          </View>
        ...
        {/* ── Hint OAuth ── */}
        <View style={s.oauthHint}>
          <Text style={s.oauthHintIcon}>🔐</Text>
          <Text style={s.oauthHintText}>
            Al abrir Claude Code por primera vez, aparecerá un botón para autorizar tu cuenta de Anthropic en el navegador.
          </Text>
        </View>
        ...
        <View style={s.sectionRow}>
          ...
          <View style={s.sectionBtns}>
            <TouchableOpacity onPress={openDlModal} style={s.pill}
              hitSlop={{ top:8, bottom:8, left:8, right:8 }}>
              <Text style={s.pillText}>+ Nuevo</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={loadProjects} style={s.pill}
              hitSlop={{ top:8, bottom:8, left:8, right:8 }}>
        ...
        <View style={s.card}>
          <Text style={s.hint}>
            Symlinks en <Text style={s.mono}>~/proyectos/</Text>
            {' · '}Toca uno para seleccionarlo como directorio de trabajo
          </Text>
          ...
          {projLoading ? (
            ...
            <View style={s.rowBetween}>
              ...
              <TouchableOpacity onPress={loadProjects}>
                <Text style={s.link}>{Icons.refresh} Reintentar</Text>
              </TouchableOpacity>
            </View>
          ) : projects.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <View style={s.projIconBox}>
                    <Text style={[s.projIconGlyph, sel && { color: theme.accent }]}>⎋</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={[s.projName, sel && { color: theme.accent }]}>
                      {proj.name}
                    </Text>
                  ...
                  {sel && (
                    <View style={s.selBadge}>
                      <Text style={s.selBadgeText}>{Icons.check} activo</Text>
                    </View>
                  )}
                  <TouchableOpacity
        ...
        <View style={s.card}>
          ...
          <InfoRow label="Fix versión"
            value=">2.1.111 usa binario nativo incompatible con Bionic"
        ...
        <View style={s.dlOverlay}>
          <View style={s.dlPanel}>
            ...
            {dlLoading ? (
              ...
              <Text style={[s.muted, { padding: 20, textAlign: 'center' }]}>
                No hay carpetas en Download
              </Text>
            ) : (
              <FlatList
                data={dlDirs}
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity
                      onPress={() => !linked && handleCreateLink(item)}
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        {linked && (
                          <Text style={[s.hint, { color: theme.success }]}>
                            symlink ya existe
                          </Text>
                        )}
                      </View>
                      {creatingLink
                        ...
                        : !linked
                          ...
                          : <Text style={[s.dlArrow, { color: theme.success }]}>
                              {Icons.check}
                            </Text>
                      }
                    </TouchableOpacity>
...
function createStyles(t) {
  return StyleSheet.create({
    ...
    termBtnIcon: {
      ...
      borderWidth: 1, borderColor: 'rgba(212,160,39,0.35)',
    ...
    oauthHint: {
      ...
      borderWidth: 1, borderColor: 'rgba(212,160,39,0.2)',
    ...
    projRow: {
      flexDirection: 'row', alignItems: 'center',
      paddingVertical: 11, gap: 10, borderRadius: 8,
    },
    projRowSel: {
      backgroundColor: 'rgba(212,160,39,0.07)',
      paddingHorizontal: 6, marginHorizontal: -6,
    },
    ...
    selBadge:  {
      ...
      borderRadius: 4, borderWidth: 1, borderColor: 'rgba(212,160,39,0.35)',
    },
    ...
    dlPanel: {
      ...
      borderTopWidth: 1, borderColor: t.border, paddingBottom: 28,
    },
    dlHeader: {
