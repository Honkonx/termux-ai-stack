...
info "Descargando los 6 scripts desde $REPO_RAW_SCRIPT/..."
echo ""

SCRIPTS_OK=0
SCRIPTS_FAIL=0

for script in install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh install_python.sh install_ssh.sh; do
  if download_file \
...
[ "$SCRIPTS_FAIL" -gt 0 ] && \
  warn "$SCRIPTS_FAIL script(s) fallaron — se re-intentará al instalar ese módulo"

# Verificación final: listar qué hay en ~/
echo ""
info "Scripts disponibles en ~/ :"
for script in menu.sh install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh install_python.sh install_ssh.sh; do
...
# Python
if check_module "python"; then
  PY_VER=$(get_reg python version)
  echo -e "  [5] Python        ${GREEN}✓ instalado${NC} v${PY_VER}  ${YELLOW}(reinstalar: 5)${NC}"
else
  echo -e "  [5] Python        ${YELLOW}○ no instalado${NC}"
fi

# SSH
if check_module "ssh"; then
  echo -e "  [6] SSH           ${GREEN}✓ instalado${NC}  ${YELLOW}(reinstalar: 6)${NC}"
else
  echo -e "  [6] SSH           ${YELLOW}○ no instalado${NC}"
fi
...
echo ""
read -r -p "  Elige [1/2/3/4/5/6/a/s]: " MODULE_CHOICE
...
echo "  SCRIPTS EN ~/:"
for f in menu.sh install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh install_python.sh install_ssh.sh; do
  [ -f "$HOME/$f" ] && \
    echo -e "  ${GREEN}✓${NC} ~/$f" || \
...
echo ""
echo "  PRÓXIMOS PASOS:"
echo "  1. El dashboard se abrirá automáticamente"
...
# Recargar .bashrc y lanzar el menú sin cerrar Termux
