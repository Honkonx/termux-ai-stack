...
_ollama_chat_full() {
  ...
  local _CHATPY="${TMPDIR:-$HOME}/_ollama_chat_$$.py"
  cat > "$_CHATPY" << 'PYEOF'
...
# Garantizar tabla existe (por si _ollama_init_db falló)
conn.execute('''CREATE TABLE IF NOT EXISTS historial (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  chat_id TEXT NOT NULL, rol TEXT NOT NULL, content TEXT NOT NULL,
  modelo TEXT, tiene_img INTEGER DEFAULT 0,
  fecha TEXT DEFAULT (datetime("now")))''')
conn.execute('CREATE INDEX IF NOT EXISTS idx_c ON historial(chat_id)')
conn.commit()

# Recuperar historial (sliding window)
rows = conn.execute(
...
# Guardar usuario + respuesta en BD
conn.execute(
...
_ollama_chat_vision() {
  ...
  local _VISIONPY="${TMPDIR:-$HOME}/_ollama_vision_$$.py"
  cat > "$_VISIONPY" << 'PYEOF'
import sys, sqlite3, os, json
from urllib import request as ureq
