...
" 2>/dev/null
    ...
    # Obtener historial reciente (sliding window)
    CONTEXT=$(python3 -c "
...
conn.close()
lines = [f\"{'Usuario' if r[0]=='user' else 'Asistente'}: {r[1]}\" for r in reversed(rows)]
print('\n'.join(lines))
" 2>/dev/null)
...
Asistente:"
    ...
    RESPONSE=$(curl -sf "$OLLAMA_URL_LOCAL/api/generate" \
      ...
      --max-time 120 \
      -d "{\"model\":\"$MODEL\",\"prompt\":$(echo "$PROMPT" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))'),\"stream\":false,\"options\":{\"num_predict\":200,\"temperature\":0.7}}" \
      2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin).get('response','').strip())" 2>/dev/null)

    echo "$RESPONSE"
    echo ""

    # Guardar en BD (sliding window — purga viejos si superan límite disco)
    python3 -c "
import sqlite3, os
conn = sqlite3.connect(os.path.expanduser('$OLLAMA_DB'))
conn.execute('INSERT INTO historial (chat_id,rol,content,modelo) VALUES (?,?,?,?)',
  ('$CHAT_ID','user',$(echo "$USER_INPUT" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read().strip()))'),None))
conn.execute('INSERT INTO historial (chat_id,rol,content,modelo) VALUES (?,?,?,?)',
  ('$CHAT_ID','assistant',$(echo "$RESPONSE" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read().strip()))') ,'$MODEL'))
# Purgar si supera límite de disco
total = conn.execute('SELECT COUNT(*) FROM historial WHERE chat_id=?',('$CHAT_ID',)).fetchone()[0]
limit = $OL_DISK_MSGS
if total > limit:
  ...
  ids = [str(r[0]) for r in oldest_id]
  conn.execute(f'DELETE FROM historial WHERE id IN ({chr(44).join(ids)})')
conn.commit(); conn.close()
" 2>/dev/null
  done
  echo ""
...
" 2>/dev/null
    ...
    if [ -n "$LAST_IMG_B64" ]; then
      ...
      # Chat de texto normal con contexto
      CONTEXT=$(python3 -c "
import sqlite3, os
conn = sqlite3.connect(os.path.expanduser('$OLLAMA_DB'))
rows = conn.execute('SELECT rol, content FROM historial WHERE chat_id=? ORDER BY id DESC LIMIT ?',
  ('$CHAT_ID', $OL_RAM_MSGS)).fetchall()
conn.close()
lines = [f\"{'Usuario' if r[0]=='user' else 'Asistente'}: {r[1]}\" for r in reversed(rows)]
print('\n'.join(lines))
" 2>/dev/null)
...
import sqlite3, os
conn = sqlite3.connect(os.path.expanduser('$OLLAMA_DB'))
conn.execute('INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
  ('$CHAT_ID','user',$(echo "$USER_INPUT" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read().strip()))'),None,$TIENE_IMG))
conn.execute('INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
  ('$CHAT_ID','assistant',$(echo "$RESPONSE" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read().strip()))') ,'$MODEL',$TIENE_IMG))
total = conn.execute('SELECT COUNT(*) FROM historial WHERE chat_id=?',('$CHAT_ID',)).fetchone()[0]
if total > $OL_DISK_MSGS:
  ids=[str(r[0]) for r in conn.execute('SELECT id FROM historial WHERE chat_id=? ORDER BY id ASC LIMIT ?',('$CHAT_ID',total-$OL_DISK_MSGS)).fetchall()]
  conn.execute(f'DELETE FROM historial WHERE id IN ({chr(44).join(ids)})')
conn.commit(); conn.close()
" 2>/dev/null
  done
...
_ollama_config_sql() {
  ...
  while true; do
    ...
    printf  "  ║  ${NC}  Actual: %-31s${CYAN}${BOLD}║\n" "${GREEN}$OL_RAM_MSGS mensajes${NC}"
...
submenu_ollama() {
  ...
  while true; do
    ...
    printf  "  ║  ${DIM}    Historial SQLite · contexto: %-9s${CYAN}${BOLD}║\n" "${OL_RAM_MSGS}msg RAM"
    echo -e "  ║  ${NC}[4] Chat con imágenes${CYAN}${BOLD}                  ║"
    echo -e "  ║  ${DIM}    Visión + historial · /imagen /camara ${CYAN}${BOLD}║"
    echo    "  ╠══════════════════════════════════════════╣"
