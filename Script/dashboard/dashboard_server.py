...
# v1.2.0 | Abril 2026
# Fix: EAS usa registry key "expo.installed", no "eas.installed"

import os, json, subprocess, collections, datetime
from http.server import HTTPServer, BaseHTTPRequestHandler
from urllib.parse import urlparse

_cmd_log = collections.deque(maxlen=10)
...
def claude_installed(d):
    # Validación triple: registry nuevo · registry viejo · wrapper en PATH
    if reg(d, "claude_code.installed", "claude.installed", "claude_installed") == "true":
        return True
    wrapper = os.path.join(
        os.environ.get("TERMUX_PREFIX", "/data/data/com.termux/files/usr"),
        "bin", "claude"
    )
    return os.path.exists(wrapper)

def eas_installed(d):
    # EAS usa "expo.installed" en el registry — no "eas.installed"
    if reg(d, "expo.installed", "eas.installed", "eas_installed", "expo_installed") == "true":
        return True
    # Fallback: verificar binario eas en PATH de Termux
    eas_bin = os.path.join(
        os.environ.get("TERMUX_PREFIX", "/data/data/com.termux/files/usr"),
        "bin", "eas"
    )
    return os.path.exists(eas_bin)
    ...
    modules = [
        {
            ...
            "name":      "n8n",
            "icon":      "⬡",
            "type":      "service",          # tiene proceso background
        ...
        },
        {
            ...
            "name":      "Claude Code",
            "icon":      "◆",
            "type":      "tool",             # sin proceso background — sin botón start/stop
    ...
    def do_POST(self):
        if urlparse(self.path).path.rstrip("/") == "/api/action":
            ...
            try:
                ...
                # Solo servicios tienen acciones — tools ignorados
                cmds = {
                    ("n8n",    "start"): "bash ~/start_servidor.sh",
                    ("n8n",    "stop"):  "pkill -f 'n8n start'; pkill -f cloudflared",
                    ("ollama", "start"): "tmux new-session -d -s ollama-server 'ollama serve'",
                ...
                if cmd:
                    ...
                    _cmd_log.append({"ts": ts, "module": module, "action": action, "cmd": cmd, "ok": True})
                    self.send_json(200, {"ok": True, "msg": f"{module} {action} iniciado"})
                else:
                    _cmd_log.append({"ts": ts, "module": module, "action": action, "cmd": "", "ok": False})
                    self.send_json(400, {"ok": False, "msg": "Acción no disponible para este módulo"})
