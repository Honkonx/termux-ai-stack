...
if check_done "claude_npm"; then
  ...
  npm install -g @anthropic-ai/claude-code 2>&1 | grep -v "^npm warn\|WARN\|deprecated" || true
  
  # Verificar que cli.js existe
  CLI_PATH_NEW="$(npm root -g 2>/dev/null)/@anthropic-ai/claude-code/cli.js"
  
  if [ ! -f "$CLI_PATH_NEW" ]; then
    error "cli.js no encontrado en $(npm root -g)/@anthropic-ai/claude-code/cli.js
    Posibles causas:
    - Error de red durante npm install
    - Permisos insuficientes en npm global
    Intenta: npm install -g @anthropic-ai/claude-code --verbose"
  fi
  
  log "cli.js encontrado: $CLI_PATH_NEW"
  
  # Verificar que funciona con node
