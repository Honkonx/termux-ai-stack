...
if check_done "claude_npm"; then
  ...
  # ── Estrategia Termux ARM64 ──────────────────────────────────
  # latest (v2.2+) usa binario nativo glibc — incompatible con Bionic libc de Termux
  # v2.1.111 incluye cli.js puro JS — única versión confirmada funcional en ARM64
  # No se intenta latest para evitar instalar la versión incorrecta
  # ─────────────────────────────────────────────────────────────
  ...
  info "Instalando v${CLAUDE_VERSION_FALLBACK} (confirmada en Termux ARM64 + Node v24)..."
  npm install -g @anthropic-ai/claude-code@${CLAUDE_VERSION_FALLBACK} 2>&1 | tail -5
  echo ""

  # Verificar que cli.js existe Y es JS válido (no wrapper bash)
  ...
  if [ -f "$CLI_PATH_NEW" ] && [ -s "$CLI_PATH_NEW" ]; then
    if node "$CLI_PATH_NEW" --version 2>&1 | grep -qv "SyntaxError"; then
      CLAUDE_OK=true
      log "v${CLAUDE_VERSION_FALLBACK} instalada y validada ✓"
    else
      warn "cli.js falla con SyntaxError — reintentando con --ignore-scripts..."
      npm cache clean --force 2>/dev/null || true
      npm install -g @anthropic-ai/claude-code@${CLAUDE_VERSION_FALLBACK} --ignore-scripts 2>&1 | tail -3
      CLI_PATH_NEW=$(find_cli)
      if [ -f "$CLI_PATH_NEW" ] && node "$CLI_PATH_NEW" --version 2>&1 | grep -qv "SyntaxError"; then
        CLAUDE_OK=true
        log "v${CLAUDE_VERSION_FALLBACK} instalada con --ignore-scripts ✓"
      fi
    fi
  fi
  ...
  if [ "$CLAUDE_OK" = "false" ]; then
    echo ""
    echo -e "${RED}[ERROR]${NC} Claude Code no instaló correctamente"
    echo ""
    echo "  Diagnóstico:"
