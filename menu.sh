...
OLLAMA_DB="$HOME/ollama_chat.db"   # default — se sobreescribe por _ollama_get_db()
OLLAMA_URL_LOCAL="http://localhost:11434"

# ── BD por modelo — genera ruta dinámica ─────────────────────
_ollama_get_db() {
  # $1 = nombre del modelo (ej: "qwen2.5:0.5b")
  # Retorna: ~/ollama_qwen2.5_0.5b.db
  local model_safe
...
# ── Obtener/crear OL_USER_ID persistente ─────────────────────
_ollama_ensure_user_id() {
  # Primera vez: pide nombre una sola vez y lo guarda en config
  # Siguientes: reutiliza el mismo ID → historial persiste entre sesiones
  if [ -z "$OL_USER_ID" ]; then
    echo ""
    echo -e "  ${CYAN}Primera vez en este equipo.${NC}"
    ...
    # Guardar en config (agrega línea sin borrar las existentes)
    # Primero eliminar línea anterior si existe, luego agregar
    ...
    echo -e "  ${GREEN}[OK]${NC} Bienvenido/a ${BOLD}$OL_USER_ID${NC} — tu historial se guardará siempre."
    echo ""
  fi
}

_ollama_save_cfg() {
  # Preserva OL_USER_ID al guardar — no lo sobreescribe
...
_ollama_chat_full() {
  ...
  _ollama_load_cfg
  _ollama_ensure_user_id
  # chat_id persistente: usuario + modelo → siempre el mismo historial
  local model_safe
  ...
  # Escribir helper Python UNA VEZ al entrar (no en cada turno)
  # Helper Python — escrito en HOME con nombre fijo (no /tmp que es noexec)
...
# Crear tabla si no existe (sin DEFAULT functions — compatibilidad SQLite ARM64)
conn.execute("""CREATE TABLE IF NOT EXISTS historial (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  chat_id TEXT NOT NULL,
  rol TEXT NOT NULL,
...
PYEOF
  ...
  echo -e "  ${DIM}Escribe '/limpiar' para borrar contexto${NC}"
...
" 2>/dev/null
    ...
    (
      while true; do
        printf "\r  ${CYAN}Bot:${NC} ${DIM}%s pensando...${NC}" "${_SPIN_CHARS:$((_SPIN_IDX % ${#_SPIN_CHARS})):1}"
    ...
    printf "\r\033[2K"   # borrar línea del spinner

    echo -e "  ${CYAN}Bot:${NC} $RESPONSE"
    echo ""
  done
...
_ollama_chat_vision() {
  ...
  _ollama_load_cfg
  _ollama_ensure_user_id
  # chat_id persistente: usuario + modelo + sufijo vision
  local model_safe
  model_safe=$(echo "$MODEL" | tr ':' '_' | tr '/' '_')
  ...
  # Helper Python para rama texto — escrito UNA VEZ fuera del loop
...
PYEOF
  ...
  echo -e "  ${DIM}Carga imagen: /imagen /sdcard/DCIM/foto.jpg${NC}"
  echo -e "  ${DIM}Luego escribe preguntas sobre la imagen${NC}"
  echo ""
  ...
  while true; do
    ...
    if [[ "$USER_INPUT" == /imagen* ]]; then
      ...
      if [ ! -f "$IMG_PATH" ]; then
        echo -e "  ${RED}[ERROR]${NC} No encontrado: $IMG_PATH"
        continue
      fi
...
" 2>/dev/null
    ...
    if [ -n "$LAST_IMG_B64" ]; then
      # Rama imagen: curl con base64 (necesita curl para campo images[])
      PROMPT_VISION="You must respond ONLY in Spanish. $USER_INPUT Responde en español."
      local _SPIN_CHARS3="⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
      local _SPIN_IDX3=0
      (
        while true; do
          printf "\r  ${CYAN}Bot:${NC} ${DIM}%s analizando imagen...${NC}" "${_SPIN_CHARS3:$((_SPIN_IDX3 % ${#_SPIN_CHARS3})):1}"
          _SPIN_IDX3=$((_SPIN_IDX3 + 1))
          sleep 0.12
        done
      ) &
      local _SPIN_PID3=$!
...
conn.execute('INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
    (chat_id,'user',user_msg,None,1))
conn.execute('INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
    (chat_id,'assistant',response,model,1))
total = conn.execute('SELECT COUNT(*) FROM historial WHERE chat_id=?',(chat_id,)).fetchone()[0]
if total > disk_msgs:
    ids=[str(r[0]) for r in conn.execute('SELECT id FROM historial WHERE chat_id=? ORDER BY id ASC LIMIT ?',(chat_id,total-disk_msgs)).fetchall()]
    conn.execute('DELETE FROM historial WHERE id IN ({})'.format(','.join(ids)))
conn.commit(); conn.close()
" "$OLLAMA_URL_LOCAL" "$MODEL" "$PROMPT_VISION" "$LAST_IMG_B64" \
  "$OLLAMA_DB" "$CHAT_ID" "$USER_INPUT" "$OL_DISK_MSGS")
      kill $_SPIN_PID3 2>/dev/null
      wait $_SPIN_PID3 2>/dev/null
      printf "\r\033[2K"
      LAST_IMG_B64=""  # consumir imagen
    else
      # Rama texto: usar helper pre-escrito
      local _SPIN_CHARS2="⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
      local _SPIN_IDX2=0
      (
        while true; do
          printf "\r  ${CYAN}Bot:${NC} ${DIM}%s pensando...${NC}" "${_SPIN_CHARS2:$((_SPIN_IDX2 % ${#_SPIN_CHARS2})):1}"
          _SPIN_IDX2=$((_SPIN_IDX2 + 1))
          sleep 0.12
        done
      ) &
      local _SPIN_PID2=$!
      RESPONSE=$(python3 "$_VISIONPY" \
        ...
        "$OL_RAM_MSGS" "$OL_DISK_MSGS" \
        "$OLLAMA_URL_LOCAL" "$USER_INPUT")
      kill $_SPIN_PID2 2>/dev/null
      wait $_SPIN_PID2 2>/dev/null
      printf "\r\033[2K"
    fi
...
_ollama_config_sql() {
  ...
  while true; do
    ...
    echo    "  ╠══════════════════════════════════════════╣"
    echo -e "  ║  ${NC}Mensajes en RAM (contexto activo)        ${CYAN}${BOLD}║"
    echo -e "  ║  ${DIM}  Más = mejor memoria · más lento         ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  Actual: ${GREEN}${OL_RAM_MSGS} mensajes${NC}${CYAN}${BOLD}                     ║"
    echo -e "  ║  ${NC}  [1] 2 msg  ~50MB   Rápido              ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [2] 4 msg  ~100MB  Normal ← recomend   ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [3] 6 msg  ~150MB  Lento               ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [4] 8 msg  ~200MB  Muy lento           ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [5] Personalizado (1-20)               ${CYAN}${BOLD}║"
    echo    "  ╠══════════════════════════════════════════╣"
    echo -e "  ║  ${NC}Mensajes en disco (historial total)      ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  Actual: ${CYAN}${OL_DISK_MSGS} mensajes${NC}${CYAN}${BOLD}                    ║"
    echo -e "  ║  ${NC}  [6] 20 msg   [7] 50 msg ← recomend    ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [8] 100 msg  [9] Sin límite            ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [0] Personalizado                      ${CYAN}${BOLD}║"
    echo    "  ╠══════════════════════════════════════════╣"
    echo -e "  ║  ${NC}[i] Ver estadísticas BD                  ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}[j] Borrar toda la BD                    ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}[b] Volver                               ${CYAN}${BOLD}║"
    echo -e "  ╚══════════════════════════════════════════╝${NC}"
    echo ""; echo -n "  Opción: "
    read -r COPT < /dev/tty
    case "$COPT" in
      1) OL_RAM_MSGS=2;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} RAM: 2 mensajes (~50MB)" ;;
      2) OL_RAM_MSGS=4;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} RAM: 4 mensajes (~100MB)" ;;
      3) OL_RAM_MSGS=6;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} RAM: 6 mensajes (~150MB)" ;;
      4) OL_RAM_MSGS=8;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} RAM: 8 mensajes (~200MB)" ;;
      5)
        echo -n "  Mensajes en contexto (1-20): "
        read -r VAL < /dev/tty
        [[ "$VAL" =~ ^[0-9]+$ ]] && [ "$VAL" -ge 1 ] && [ "$VAL" -le 20 ] \
          && { OL_RAM_MSGS=$VAL; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} RAM: $VAL mensajes"; } \
          || echo -e "  ${RED}[ERROR]${NC} Valor inválido (1-20)" ;;
      6) OL_DISK_MSGS=20;   _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 20 mensajes" ;;
      7) OL_DISK_MSGS=50;   _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 50 mensajes" ;;
      8) OL_DISK_MSGS=100;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 100 mensajes" ;;
  ...
  while true; do
    ...
    case "$OPT" in
      ...
      4)
        clear; echo ""
        _ollama_ensure_server || { read -r _ < /dev/tty; continue; }
        echo -e "  ${CYAN}Modelos de imagen disponibles:${NC}"
        _ollama_pick_model "vision"
        if [ -z "$PICKED_MODEL" ]; then
          echo ""
          echo -e "  ${BOLD}¿Descargar modelo de imagen ahora?${NC}"; echo ""
      ...
      5)
        while true; do
          ...
          echo -e "  ╠══════════════════════════════════════════╣${NC}"
          # Listar modelos con nombre y tamaño alineados
          ollama list 2>/dev/null | tail -n +2 | \
            awk '{printf "  ║  %-26s  %s %s\n", $1, $3, $4}' | head -12
          echo ""
          echo -e "${CYAN}${BOLD}  ╠══════════════════════════════════════════╣"
          echo -e "  ║  ${NC}[d] Descargar modelo                    ${CYAN}${BOLD}║"
          echo -e "  ║  ${NC}[r] Eliminar modelo                     ${CYAN}${BOLD}║"
          echo -e "  ║  ${NC}[b] Volver                              ${CYAN}${BOLD}║"
          echo -e "  ╚══════════════════════════════════════════╝${NC}"
          echo ""; echo -n "  Opción: "
          read -r MOPT < /dev/tty
          case "$MOPT" in
            d|D)
              ...
              echo    "  ╠══════════════════════════════════════════╣"
              echo -e "  ║  ${NC}  [9] Escribir nombre manualmente        ${CYAN}${BOLD}║"
              echo -e "  ║  ${NC}  [b] Cancelar                           ${CYAN}${BOLD}║"
              echo -e "  ╚══════════════════════════════════════════╝${NC}"
              echo ""; echo -n "  Elige: "
              read -r DC < /dev/tty
              case "$DC" in
                ...
                5) DL="llama3.2:3b"     ;;
                6) DL="moondream:1.8b"  ;;
                7) DL="llava-phi3:3.8b" ;;
                8) DL="llava:7b"        ;;
                9) echo -n "  Nombre del modelo: "; read -r DL < /dev/tty ;;
                b|B|"") DL="" ;;
                *) DL="" ;;
              esac
...
submenu_opencode() {
  ...
  while true; do
    ...
    case "$OPT" in
      ...
      2) # Servidor web — lanzar visible, Enter para volver al menú
        clear; echo ""
...
submenu_trading() {
  while true; do
    ...
    if [ -f "$TRADING_DB" ]; then
      SENALES_TOTAL=$(python3 -c "import sqlite3; conn=sqlite3.connect('$TRADING_DB'); c=conn.cursor(); c.execute('SELECT COUNT(*) FROM senales'); print(c.fetchone()[0]); conn.close()" 2>/dev/null || echo "?")
      WIN=$(python3 -c "import sqlite3; conn=sqlite3.connect('$TRADING_DB'); c=conn.cursor(); c.execute(\"SELECT COUNT(*) FROM senales WHERE resultado='WIN'\"); print(c.fetchone()[0]); conn.close()" 2>/dev/null || echo "?")
      LOSS=$(python3 -c "import sqlite3; conn=sqlite3.connect('$TRADING_DB'); c=conn.cursor(); c.execute(\"SELECT COUNT(*) FROM senales WHERE resultado='LOSS'\"); print(c.fetchone()[0]); conn.close()" 2>/dev/null || echo "?")
      PENDIENTES=$(python3 -c "import sqlite3; conn=sqlite3.connect('$TRADING_DB'); c=conn.cursor(); c.execute(\"SELECT COUNT(*) FROM senales WHERE resultado='PENDIENTE'\"); print(c.fetchone()[0]); conn.close()" 2>/dev/null || echo "?")
    ...
    echo    "  ╠══════════════════════════════════════════╣"
    printf  "  ║  BD: %-35b║\n" "$DB_STATUS"
    printf  "  ║  %-40s║\n" "Total:$SENALES_TOTAL  W:$WIN  L:$LOSS  Pend:$PENDIENTES"
    echo -e "  ╚══════════════════════════════════════════╝${NC}"
    echo ""
