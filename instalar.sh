...
info "Descargando los 4 scripts desde $REPO_RAW_SCRIPT/..."
echo ""

SCRIPTS_OK=0
SCRIPTS_FAIL=0

for script in install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh; do
  if download_file \
...
echo ""
log "$SCRIPTS_OK/4 scripts descargados correctamente"
[ "$SCRIPTS_FAIL" -gt 0 ] && \
...
for script in menu.sh install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh; do
