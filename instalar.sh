...
info "Descargando los 5 scripts desde $REPO_RAW_SCRIPT/..."
echo ""

SCRIPTS_OK=0
SCRIPTS_FAIL=0

for script in install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh install_python.sh; do
  if download_file \
...
echo ""
log "$SCRIPTS_OK/5 scripts descargados correctamente"
[ "$SCRIPTS_FAIL" -gt 0 ] && \
...
for script in menu.sh install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh install_python.sh; do
...
echo ""
read -r -p "  Elige [1/2/3/4/5/a/s]: " MODULE_CHOICE
...
echo "  SCRIPTS EN ~/:"
for f in menu.sh install_n8n.sh install_claude.sh install_ollama.sh install_expo.sh install_python.sh; do
  [ -f "$HOME/$f" ] && \
    echo -e "  ${GREEN}✓${NC} ~/$f" || \
