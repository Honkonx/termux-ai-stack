...
check_remote() {
  ...
  # ── REGLA DE DETECCIÓN ────────────────────────────────────────
  # Remote = módulo configurado por install_remote.sh
  # NO basta con que openssh esté instalado — Termux lo incluye siempre
  # como dependencia base (con Port 8022 por defecto en sshd_config)
  #
  # Remote se considera instalado SOLO si:
  #   A) El registry tiene ssh.installed=true (install_remote.sh lo escribió)
  #   B) O dashboard_server.py existe en HOME (install_remote.sh lo descargó)
  #
  # El binario sshd + sshd_config con Port 8022 NO cuenta
  # porque el pkg openssh de Termux ya lo instala así por defecto
  # ─────────────────────────────────────────────────────────────

  local has_remote=false

  # Condición A: registry explícito de install_remote.sh
  if [ "$ssh_installed" = "true" ] || [ "$dashboard_installed" = "true" ]; then
    has_remote=true
  fi

  # Condición B: dashboard_server.py existe (descargado por install_remote.sh)
  if [ -f "$HOME/dashboard_server.py" ] || [ -f "$HOME/dashboard/dashboard_server.py" ]; then
    has_remote=true
  fi
...
_n8n_repair_scripts() {
  ...
  # Verificar que n8n esté instalado en proot
...
SCRIPT
  ...
  echo ""
  echo -e "  ${GREEN}[OK]${NC} 7 scripts regenerados correctamente"
...
# ════════════════════════════════════════════
#  SUBMENÚ OLLAMA v2 — Chat rápido / completo / imágenes
# ════════════════════════════════════════════
...
_ollama_list_vision_models() {
  # Modelos conocidos de visión — filtra de los instalados
  ollama list 2>/dev/null | tail -n +2 | awk '{print $1}' | grep -E "moondream|llava|bakllava|llama3.2-vision|minicpm-v|qwen.*vl|gemma.*vision" | grep -v "^$"
}

_ollama_pick_model() {
  # $1 = "vision" para filtrar solo modelos de visión, "" para todos
  local filter="$1"
  ...
  if [ ${#MODELS[@]} -eq 0 ]; then
    if [ "$filter" = "vision" ]; then
      ...
      echo -e "  ${YELLOW}[AVISO]${NC} No hay modelos instalados. Ve a [4]."
    fi
    PICKED_MODEL=""
    return 1
  fi
...
_ollama_save_cfg() {
  echo "OL_RAM_MSGS=$OL_RAM_MSGS" > "$OLLAMA_CFG"
  echo "OL_DISK_MSGS=$OL_DISK_MSGS" >> "$OLLAMA_CFG"
}
...
# ── Chat completo con historial SQLite sliding window ──────────
_ollama_chat_full() {
  local MODEL="$1"
  local CHAT_ID="chat_$(date +%s)"
  _ollama_load_cfg
  _ollama_init_db
  ...
  echo -e "  ${DIM}Escribe 'salir' o '/bye' para terminar${NC}"
  echo -e "  ${DIM}Escribe '/limpiar' para borrar contexto activo${NC}"
  echo -e "  ${DIM}Escribe '/stats' para ver estadísticas${NC}"
  echo ""
...
all_chats = conn.execute('SELECT COUNT(DISTINCT chat_id) FROM historial').fetchone()[0]
all_msgs  = conn.execute('SELECT COUNT(*) FROM historial').fetchone()[0]
conn.close()
print(f'  Chat actual: {total} mensajes')
print(f'  Total en BD: {all_msgs} mensajes en {all_chats} chats')
" 2>/dev/null
      continue
    fi

    # ── FIX BUG 1: prompt y guardado via Python puro (sin piping shell) ──
    # USER_INPUT se pasa como argv[1] — evita contaminación por expansión bash
    _TMPPY="/tmp/_ollama_turn_$$.py"
    cat > "$_TMPPY" << 'PYEOF'
...
url       = sys.argv[6]
user_msg  = sys.argv[7]   # mensaje limpio, sin riesgo de expansión
...
# 2. Construir prompt con sliding window
lines = [
    ('Usuario' if r[0] == 'user' else 'Asistente') + ': ' + r[1]
...
try:
    ...
    response = '[ERROR] No se pudo conectar con Ollama'

print(response)

# 4. Guardar ambos mensajes en BD
conn.execute(
    'INSERT INTO historial (chat_id,rol,content,modelo) VALUES (?,?,?,?)',
    (chat_id, 'user', user_msg, None))
conn.execute(
    'INSERT INTO historial (chat_id,rol,content,modelo) VALUES (?,?,?,?)',
    (chat_id, 'assistant', response, model))

# 5. Purgar si supera límite disco
total = conn.execute(
...
PYEOF
    ...
    echo -en "  ${CYAN}Bot:${NC} "
    RESPONSE=$(python3 "$_TMPPY" \
      "$OLLAMA_DB" "$CHAT_ID" "$MODEL" \
      "$OL_RAM_MSGS" "$OL_DISK_MSGS" \
      "$OLLAMA_URL_LOCAL" "$USER_INPUT" 2>/dev/null)
    rm -f "$_TMPPY"
  ...
  echo ""
  echo -e "  ${DIM}Chat guardado. ID: $CHAT_ID${NC}"
  echo ""; read -r _ < /dev/tty
}
...
_ollama_chat_vision() {
  local MODEL="$1"
  local CHAT_ID="vision_$(date +%s)"
  _ollama_load_cfg
  _ollama_init_db

  clear; echo ""
  echo -e "${CYAN}${BOLD}  ╔══════════════════════════════════════════╗"
  echo -e "  ║  🖼  Chat visión — $MODEL"
  echo -e "  ╠══════════════════════════════════════════╣${NC}"
  echo -e "  Contexto: ${GREEN}$OL_RAM_MSGS mensajes${NC} en RAM"
  echo -e "  ${DIM}Comandos: 'salir' · '/imagen RUTA' · '/camara' · '/stats'${NC}"
  echo -e "  ${DIM}Para analizar imagen: escribe '/imagen /sdcard/DCIM/foto.jpg'${NC}"
  echo -e "  ${DIM}Luego puedes hacer preguntas sobre ella${NC}"
  echo ""
  ...
  while true; do
    ...
    if [[ "$USER_INPUT" == /imagen* ]]; then
      IMG_PATH="${USER_INPUT#/imagen }"
      IMG_PATH="${IMG_PATH//\'/}"
      IMG_PATH=$(eval echo "$IMG_PATH" 2>/dev/null || echo "$IMG_PATH")
      if [ ! -f "$IMG_PATH" ]; then
        echo -e "  ${RED}[ERROR]${NC} Archivo no encontrado: $IMG_PATH"
        echo -e "  ${DIM}Ejemplo: /imagen /sdcard/DCIM/Camera/foto.jpg${NC}"
        continue
      fi
      # Redimensionar si >500KB
      IMG_BYTES=$(stat -c%s "$IMG_PATH" 2>/dev/null || echo 0)
      WORK_IMG="$IMG_PATH"
      if [ "$IMG_BYTES" -gt 500000 ]; then
        echo -en "  Redimensionando imagen grande..."
        python3 -c "
...
print(f' {w}x{h}→{img.size[0]}x{img.size[1]} ({os.path.getsize(out)} bytes)')
" 2>/dev/null && WORK_IMG="$HOME/vision_tmp.jpg" || echo " (sin Pillow, usando original)"
      fi
      LAST_IMG_B64=$(base64 -w 0 "$WORK_IMG" 2>/dev/null)
      LAST_IMG_PATH="$IMG_PATH"
      [ -n "$LAST_IMG_B64" ] \
        && echo -e "  ${GREEN}[OK]${NC} Imagen cargada: $(basename "$IMG_PATH") — ahora puedes hacer preguntas" \
        || { echo -e "  ${RED}[ERROR]${NC} No se pudo leer la imagen"; LAST_IMG_B64=""; }
      continue
    fi

    # Comando /camara — busca última foto de la cámara
    if [ "$USER_INPUT" = "/camara" ]; then
      ...
      if [ -n "$LAST_CAM" ]; then
        ...
        read -r _UC < /dev/tty
        if [ "$_UC" = "s" ] || [ "$_UC" = "S" ]; then
          USER_INPUT="/imagen $LAST_CAM"
          # Re-procesar como /imagen
          IMG_PATH="$LAST_CAM"
          IMG_BYTES=$(stat -c%s "$IMG_PATH" 2>/dev/null || echo 0)
          WORK_IMG="$IMG_PATH"
          if [ "$IMG_BYTES" -gt 500000 ]; then
...
" 2>/dev/null && WORK_IMG="$HOME/vision_tmp.jpg" || echo ""
          fi
          LAST_IMG_B64=$(base64 -w 0 "$WORK_IMG" 2>/dev/null)
          LAST_IMG_PATH="$IMG_PATH"
        ...
        echo -e "  ${YELLOW}[AVISO]${NC} No se encontraron fotos en /sdcard/DCIM/Camera"
      fi
      continue
    fi
...
" 2>/dev/null
    ...
    # Enviar a Ollama — con imagen si hay una cargada
    echo -en "  ${CYAN}Bot:${NC} "

    if [ -n "$LAST_IMG_B64" ]; then
      PROMPT_VISION="You must respond ONLY in Spanish. $USER_INPUT Responde en español."
      RESPONSE=$(curl -sf "$OLLAMA_URL_LOCAL/api/generate" \
        -H "Content-Type: application/json" \
        --max-time 300 \
        -d "{\"model\":\"$MODEL\",\"prompt\":$(echo "$PROMPT_VISION" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read().strip()))'),\"images\":[\"$LAST_IMG_B64\"],\"stream\":false,\"options\":{\"num_predict\":200,\"temperature\":0.1}}" \
        2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin).get('response','').strip())" 2>/dev/null)
      TIENE_IMG=1
      LAST_IMG_B64=""  # consumir imagen después de primera pregunta
    else
      # ── FIX BUG 1 (vision — rama texto): helper Python via argv ──
      _TMPPY2="/tmp/_ollama_vturn_$$.py"
      cat > "$_TMPPY2" << 'PYEOF'
import sys, sqlite3, os, json
from urllib import request as ureq
...
conn.execute(
    'INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
    (chat_id, 'user', user_msg, None, 0))
conn.execute(
    'INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
    (chat_id, 'assistant', response, model, 0))
...
PYEOF
    ...
    # Guardar visión (solo cuando había imagen — rama texto ya guardó arriba)
    if [ "$TIENE_IMG" = "1" ]; then
      python3 - << PYEOF 2>/dev/null
import sqlite3, os, json, sys

db_path   = os.path.expanduser('$OLLAMA_DB')
chat_id   = '$CHAT_ID'
model     = '$MODEL'
disk_msgs = $OL_DISK_MSGS
user_msg  = sys.stdin.read()  # no se usa aquí, datos ya en vars bash

conn = sqlite3.connect(db_path)
# Usar variables bash directamente via here-doc interpolación
conn.execute(
    'INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
    (chat_id, 'user', """$USER_INPUT""", None, 1))
conn.execute(
    'INSERT INTO historial (chat_id,rol,content,modelo,tiene_img) VALUES (?,?,?,?,?)',
    (chat_id, 'assistant', """$RESPONSE""", model, 1))
total = conn.execute(
    'SELECT COUNT(*) FROM historial WHERE chat_id=?', (chat_id,)).fetchone()[0]
if total > disk_msgs:
    ids = [str(r[0]) for r in conn.execute(
...
PYEOF
  ...
  rm -f "$HOME/vision_tmp.jpg" 2>/dev/null
  echo ""; read -r _ < /dev/tty
}
...
_ollama_config_sql() {
  ...
  while true; do
    clear; echo ""
    echo -e "${CYAN}${BOLD}  ╔══════════════════════════════════════════╗"
    echo    "  ║  ⚙  Configuración — Historial SQLite    ║"
    echo    "  ╠══════════════════════════════════════════╣"
    echo -e "  ║  ${NC}Mensajes activos (contexto de la IA)     ${CYAN}${BOLD}║"
    echo -e "  ║  ${DIM}  Más = mejor contexto, más lento        ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  Actual: ${GREEN}$OL_RAM_MSGS mensajes${NC}"
    echo -e "  ║  ${NC}  [2] 2 msg  ~50MB    Rápido             ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [4] 4 msg  ~100MB   Normal ← recomend  ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [6] 6 msg  ~150MB   Lento              ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [8] 8 msg  ~200MB   Muy lento          ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [c] Personalizado (1-20)               ${CYAN}${BOLD}║"
    echo    "  ╠══════════════════════════════════════════╣"
    echo -e "  ║  ${NC}Mensajes archivados en disco             ${CYAN}${BOLD}║"
    echo -e "  ║  ${DIM}  Se usan para historial y /stats         ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  Actual: ${CYAN}$OL_DISK_MSGS mensajes${NC}"
    echo -e "  ║  ${NC}  [d] 20 · [e] 50 · [f] 100 · [g] ∞     ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}  [h] Personalizado                      ${CYAN}${BOLD}║"
    echo    "  ╠══════════════════════════════════════════╣"
    echo -e "  ║  ${NC}[i] Ver BD actual   [j] Limpiar toda BD  ${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}[b] Volver                               ${CYAN}${BOLD}║"
    echo -e "  ╚══════════════════════════════════════════╝${NC}"
    echo ""; echo -n "  Opción: "
    read -r COPT < /dev/tty
    case "$COPT" in
      2) OL_RAM_MSGS=2;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 2 mensajes (~50MB RAM)" ;;
      4) OL_RAM_MSGS=4;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 4 mensajes (~100MB RAM)" ;;
      6) OL_RAM_MSGS=6;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 6 mensajes (~150MB RAM)" ;;
      8) OL_RAM_MSGS=8;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 8 mensajes (~200MB RAM)" ;;
      c|C)
        echo -n "  Mensajes en contexto (1-20): "
        read -r VAL < /dev/tty
        [[ "$VAL" =~ ^[0-9]+$ ]] && [ "$VAL" -ge 1 ] && [ "$VAL" -le 20 ] \
          && { OL_RAM_MSGS=$VAL; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: $VAL mensajes"; } \
          || echo -e "  ${RED}[ERROR]${NC} Valor inválido (1-20)" ;;
      d|D) OL_DISK_MSGS=20;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 20 mensajes" ;;
      e|E) OL_DISK_MSGS=50;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 50 mensajes" ;;
      f|F) OL_DISK_MSGS=100; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 100 mensajes" ;;
      g|G) OL_DISK_MSGS=9999; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: sin límite" ;;
      h|H)
        echo -n "  Mensajes en disco (10-9999): "
        read -r VAL < /dev/tty
        [[ "$VAL" =~ ^[0-9]+$ ]] && [ "$VAL" -ge 10 ] \
          && { OL_DISK_MSGS=$VAL; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: $VAL mensajes"; } \
...
print(f'  Chats únicos   : {chats}')
print(f'  Con imagen     : {imgs}')
print(f'  Tamaño BD      : {size} bytes')
if modelos:
    print('  Por modelo:')
    for m,c in modelos: print(f'    {m}: {c} respuestas')
" 2>/dev/null || echo "  BD no existe aún" ;;
      j|J)
        echo -n "  ¿Borrar TODA la BD de chat? (s/n): "
        read -r _DEL < /dev/tty
        [ "$_DEL" = "s" ] || [ "$_DEL" = "S" ] && {
          rm -f "$OLLAMA_DB" && echo -e "  ${GREEN}[OK]${NC} BD eliminada" || echo -e "  ${RED}[ERROR]${NC}"
        } ;;
      b|B|"") break ;;
    esac
    sleep 1
...
submenu_ollama() {
  ...
  while true; do
    ...
    # ── Header minimalista (sin marcos — adaptado a móvil) ──
    if [ "$state" = "running" ]; then
      echo -e "  ${CYAN}${BOLD}◎ OLLAMA${NC}  ${GREEN}● activo${NC}  :11434"
    else
      echo -e "  ${CYAN}${BOLD}◎ OLLAMA${NC}  ${YELLOW}○ detenido${NC}"
    fi
    echo -e "  ${DIM}──────────────────────────────${NC}"
    echo -e "  ${BOLD}[1]${NC} Iniciar / detener"
    echo ""
    echo -e "  ${BOLD}[2]${NC} Chat rápido"
    echo -e "  ${DIM}    Sin historial${NC}"
    echo -e "  ${BOLD}[3]${NC} Chat completo"
    echo -e "  ${DIM}    SQLite · ${OL_RAM_MSGS} msg RAM · ${OL_DISK_MSGS} disco${NC}"
    echo -e "  ${BOLD}[4]${NC} Chat imágenes"
    echo -e "  ${DIM}    /imagen RUTA · /camara${NC}"
    echo -e "  ${DIM}──────────────────────────────${NC}"
    echo -e "  ${BOLD}[5]${NC} Modelos"
    echo -e "  ${BOLD}[6]${NC} Config historial"
    ...
    case "$OPT" in
      ...
      1)
        ...
        if [ "$state" = "running" ]; then
          ...
          [ "$_ST" = "s" ] || [ "$_ST" = "S" ] && {
            ...
            sleep 1; echo -e "  ${GREEN}[OK]${NC} Servidor detenido"
            state="stopped"
          }
        else
      ...
      2)
        ...
        _ollama_pick_model || { read -r _ < /dev/tty; continue; }
        MODEL="$PICKED_MODEL"
        echo -e "  ${GREEN}[OK]${NC} Chat rápido con ${CYAN}$MODEL${NC}"
        echo -e "  ${DIM}Sin historial — Ctrl+D o /bye para salir${NC}"; echo ""
        ollama run "$MODEL" < /dev/tty
        echo ""; read -r _ < /dev/tty ;;
      ...
      4)
        clear; echo ""
        _ollama_ensure_server || { read -r _ < /dev/tty; continue; }
        echo -e "  ${BOLD}Modelos de visión instalados:${NC}"; echo ""
        _ollama_pick_model "vision"
        if [ -z "$PICKED_MODEL" ]; then
          echo ""
          echo -e "  ${BOLD}¿Descargar modelo de visión ahora?${NC}"
          echo "    [1] llava-phi3:3.8b  ~2.5GB  ← RECOMENDADO (mejor idioma)"
          echo "    [2] moondream:1.8b   ~1.1GB  (más rápido, idioma variable)"
          echo "    [s] Cancelar"
          echo ""; echo -n "  Elige: "
          read -r _DV < /dev/tty
          case "$_DV" in
      ...
      5)
        while true; do
          ...
          case "$MOPT" in
            d|D)
              clear; echo ""
              echo -e "  ${CYAN}Modelos recomendados:${NC}"; echo ""
              echo "  TEXTO:"
              echo "    [a] qwen2.5:0.5b    ~397MB  — liviano"
              echo "    [b] qwen2.5:1.5b    ~986MB  — balance"
              echo "    [c] qwen2.5:3b      ~1.9GB  — buena calidad"
              echo "    [d] gemma3:4b       ~2.5GB  — excelente"
              echo "    [e] llama3.2:3b     ~2.0GB  — multilingüe"
              echo ""
              echo "  VISIÓN (imágenes):"
              echo "    [f] moondream:1.8b  ~1.1GB  — rápido"
              echo "    [g] llava-phi3:3.8b ~2.5GB  — mejor calidad ← RECOMENDADO"
              echo "    [h] llava:7b        ~4.7GB  — alta calidad"
              echo ""
              echo "    [m] Escribir manualmente"
              echo ""; echo -n "  Elige: "
              read -r DC < /dev/tty
              case "$DC" in
                a|A) DL="qwen2.5:0.5b"    ;;  b|B) DL="qwen2.5:1.5b"   ;;
                c|C) DL="qwen2.5:3b"      ;;  d|D) DL="gemma3:4b"       ;;
                e|E) DL="llama3.2:3b"     ;;  f|F) DL="moondream:1.8b"  ;;
                g|G) DL="llava-phi3:3.8b" ;;  h|H) DL="llava:7b"        ;;
                m|M) echo -n "  Nombre: "; read -r DL < /dev/tty ;;
                *) DL="" ;;
              esac
    ...
    _ollama_load_cfg  # refrescar config después de cada acción
  done
}
...
submenu_claude() {
 ...
 read -r OPT < /dev/tty
    ...
    case "$OPT" in
      ...
      2)
        ...
        echo ""; echo -n "  Elige opción: "
 read -r PCHOICE < /dev/tty

        local TARGET_DIR=""
        case "$PCHOICE" in
          m|M) echo -n "  Ruta del proyecto: "
 ...
 read -r DCHOICE < /dev/tty
      ...
      3)
        # Submenú gestionar proyectos / symlinks
        while true; do
          clear; echo ""
          echo -e "${CYAN}${BOLD}  ╔══════════════════════════════════════════╗"
          echo    "  ║  ◆ CLAUDE CODE — Proyectos              ║"
          echo    "  ╠══════════════════════════════════════════╣"
          echo -e "  ║  ${NC}[1] Listar proyectos (symlinks)${CYAN}${BOLD}        ║"
          echo -e "  ║  ${NC}[2] Crear symlink desde Download${CYAN}${BOLD}       ║"
          echo -e "  ║  ${NC}[3] Eliminar symlink${CYAN}${BOLD}                   ║"
          echo -e "  ║  ${NC}[b] Volver${CYAN}${BOLD}                             ║"
          echo -e "  ╚══════════════════════════════════════════╝${NC}"
          echo ""; echo -n "  Opción: "
          read -r GOPT < /dev/tty
          case "$GOPT" in
            1) # Listar proyectos / symlinks
              clear; echo ""
              echo -e "  ${BOLD}Proyectos en ~/proyectos/:${NC}"; echo ""
              mkdir -p "$HOME/proyectos"
              if ls "$HOME/proyectos/" 2>/dev/null | grep -q .; then
                ls -la "$HOME/proyectos/" 2>/dev/null
              else
                echo -e "  ${DIM}(directorio vacío — usa [2] para agregar proyectos)${NC}"
              fi
              echo ""; read -r _ < /dev/tty ;;
            2) # Crear symlink desde Download
              clear; echo ""
              echo -e "  ${BOLD}Crear symlink desde /sdcard/Download/${NC}"; echo ""
              ...
              for i in "${!DL_DIRS[@]}"; do
                LINK_DST="$HOME/proyectos/${DL_DIRS[$i]}"
                [ -L "$LINK_DST" ] && printf "    [%d] %s ${DIM}(ya existe symlink)${NC}
...
" "$((i+1))" "${DL_DIRS[$i]}"
              ...
              read -r DCHOICE < /dev/tty
              if [[ "$DCHOICE" =~ ^[0-9]+$ ]] && [ "$DCHOICE" -ge 1 ] && [ "$DCHOICE" -le "${#DL_DIRS[@]}" ]; then
                DNAME="${DL_DIRS[$((DCHOICE-1))]}"
                LINK_SRC="/storage/emulated/0/Download/${DNAME}"
                LINK_DST="$HOME/proyectos/${DNAME}"
                mkdir -p "$HOME/proyectos"
                if [ -L "$LINK_DST" ]; then
                  echo -e "  ${YELLOW}[AVISO]${NC} Ya existe: ~/proyectos/${DNAME}"
                else
                  ln -s "$LINK_SRC" "$LINK_DST" 2>/dev/null &&                     echo -e "  ${GREEN}[OK]${NC} Symlink creado: ~/proyectos/${DNAME} → $LINK_SRC" ||                     echo -e "  ${RED}[ERROR]${NC} No se pudo crear el symlink"
                fi
              fi
              echo ""; read -r _ < /dev/tty ;;
            3) # Eliminar symlink
              clear; echo ""
              echo -e "  ${BOLD}Eliminar symlink de ~/proyectos/${NC}"; echo ""
              mkdir -p "$HOME/proyectos"
              mapfile -t LINKS < <(find "$HOME/proyectos" -maxdepth 1 -type l 2>/dev/null | xargs -I{} basename {})
              [ ${#LINKS[@]} -eq 0 ] && {
                echo -e "  ${DIM}No hay symlinks en ~/proyectos/${NC}"
                read -r _ < /dev/tty; continue
              }
...
" "$((i+1))" "${LINKS[$i]}"; done
              ...
              if [[ "$LCHOICE" =~ ^[0-9]+$ ]] && [ "$LCHOICE" -ge 1 ] && [ "$LCHOICE" -le "${#LINKS[@]}" ]; then
                LNAME="${LINKS[$((LCHOICE-1))]}"
                echo -n "  ¿Eliminar ~/proyectos/${LNAME}? (s/n): "
                read -r LCONFIRM < /dev/tty
                [ "$LCONFIRM" = "s" ] || [ "$LCONFIRM" = "S" ] && {
                  rm "$HOME/proyectos/$LNAME" &&                     echo -e "  ${GREEN}[OK]${NC} Symlink eliminado" ||                     echo -e "  ${RED}[ERROR]${NC} No se pudo eliminar"
                }
              fi
              echo ""; read -r _ < /dev/tty ;;
            b|B|"") break ;;
 ...
 read -r DCHOICE < /dev/tty
        ...
        if [ -n "$NEW_PROJ" ]; then
          ...
          if [ ! -d "$REAL_PATH" ]; then
            ...
            echo -n "  ¿Guardar de todas formas? (s/n): "
            read -r FC < /dev/tty
      ...
      7)
        ...
        [ ! -d "$REAL_PATH/.git" ] && {
          ...
          echo -n "  ¿Inicializar git ahora? (s/n): "
          read -r INITGIT < /dev/tty
...
" "$((i+1))" "${DBS[$i]}"; done
        ...
        read -r CHOICE < /dev/tty
        if [[ "$CHOICE" =~ ^[0-9]+$ ]] && [ "$CHOICE" -ge 1 ] && [ "$CHOICE" -le "${#DBS[@]}" ]; then
          DB_PATH="${DBS[$((CHOICE-1))]}"
          CSV_DIR="$(dirname "$DB_PATH")"
          echo -e "
  ${CYAN}Tablas en $DB_PATH:${NC}"; echo ""
          mapfile -t TABLES < <(sqlite3 "$DB_PATH" ".tables" 2>/dev/null | tr ' ' '
' | grep -v "^$")
          [ ${#TABLES[@]} -eq 0 ] && { echo -e "  ${YELLOW}No hay tablas.${NC}"; read -r _ < /dev/tty; continue; }
          for i in "${!TABLES[@]}"; do printf "    [%d] %s
" "$((i+1))" "${TABLES[$i]}"; done
          echo "    [a] Todas las tablas"
          echo ""; echo -n "  Tabla a exportar: "
          read -r TCHOICE < /dev/tty
          export_table() {
            local tbl="$1"
            local out="$CSV_DIR/${tbl}.csv"
            sqlite3 -csv "$DB_PATH" "SELECT * FROM ${tbl};" > "$out" 2>/dev/null &&               echo -e "  ${GREEN}[OK]${NC} $tbl → $out" ||               echo -e "  ${RED}[ERROR]${NC} No se pudo exportar $tbl"
          }
          if [ "$TCHOICE" = "a" ] || [ "$TCHOICE" = "A" ]; then
            for tbl in "${TABLES[@]}"; do export_table "$tbl"; done
          elif [[ "$TCHOICE" =~ ^[0-9]+$ ]] && [ "$TCHOICE" -ge 1 ] && [ "$TCHOICE" -le "${#TABLES[@]}" ]; then
            export_table "${TABLES[$((TCHOICE-1))]}"
          fi
        fi
        echo ""; read -r _ < /dev/tty ;;
      6) # Crear nueva BD vacía
        clear; echo ""
        echo -e "  ${BOLD}Crear nueva BD SQLite vacía${NC}"; echo ""
        echo -e "  ${DIM}Se creará en ~/  (ejemplo: misdatos)${NC}"; echo ""
        echo -n "  Nombre de la BD (sin extensión): "
        read -r DB_NAME < /dev/tty
        if [ -z "$DB_NAME" ]; then
          echo -e "  ${YELLOW}Cancelado.${NC}"
        else
          DB_FILE="$HOME/${DB_NAME}.db"
          sqlite3 "$DB_FILE" "" 2>/dev/null &&             echo -e "  ${GREEN}[OK]${NC} BD creada: $DB_FILE" ||             echo -e "  ${RED}[ERROR]${NC} No se pudo crear la BD"
        fi
        echo ""; read -r _ < /dev/tty ;;
      b|B|"") break ;;
    esac
    ...
    case "$OPT" in
      ...
      6)
        ...
        if [ ${#PY_SCRIPTS[@]} -eq 0 ]; then
          ...
          echo -n "  Escribe la ruta completa: "
          read -r MANUAL_PY < /dev/tty
          ...
          echo -n "  Elige número: "
          read -r SCHOICE < /dev/tty
          [ "$SCHOICE" = "m" ] || [ "$SCHOICE" = "M" ] && {
            echo -n "  Ruta: "
            read -r MANUAL_PY < /dev/tty; PY_SCRIPTS=("$MANUAL_PY"); SCHOICE=1
          }
...
DBSTART_AUTO
      ...
      8) # Detener Dashboard
        ...
        if pgrep -f "dashboard_server.py" &>/dev/null; then
          ...
          sleep 1  # esperar a que el proceso muera antes de verificar
          if pgrep -f "dashboard_server.py" &>/dev/null; then
            pkill -9 -f "dashboard_server.py" 2>/dev/null
            sleep 1
          fi
          pgrep -f "dashboard_server.py" &>/dev/null &&             echo -e "  ${RED}[ERROR]${NC} No se pudo detener el dashboard" ||             echo -e "  ${GREEN}[OK]${NC} Dashboard detenido"
        else
          echo -e "  ${YELLOW}[INFO]${NC} Dashboard no estaba corriendo"
      ...
      t|T) # Configurar token CF-SSH
        ...
        echo ""; echo "  (ENTER para cancelar)"
        echo -n "  Nuevo token CF-SSH: "
        read -r NEW_CF_SSH < /dev/tty
...
uninstall_module() {
  ...
  echo -e "  ╚══════════════════════════════════════════╝${NC}"; echo ""
  echo -n "  ¿Confirmar? (escribe SI para confirmar): "
      ...
      while true; do
        ...
        case "$POPT" in
          ...
          2|a|A)
            ...
            # Alias termux-boost
            if ! grep -q "alias termux-boost" "$BASHRC" 2>/dev/null; then
              cat >> "$BASHRC" << 'ALIAS_BOOST'

# ── termux-ai-stack: aliases de rendimiento ──
alias termux-boost='termux-wake-lock 2>/dev/null; echo "[OK] Wake-lock activado (CPU sin throttling)"'
...
ALIAS_BOOST
            ...
            _perf_ok "OLLAMA_MAX_LOADED_MODELS=1 — evita OOM con múltiples modelos"
            _perf_ok "OLLAMA_NUM_PARALLEL=1 — conserva RAM en ARM64"
            _perf_ok "OLLAMA_FLASH_ATTENTION=1 — atencion más eficiente"
            _perf_ok "alias termux-boost — activa wake-lock + prioridad CPU"
            _perf_ok "alias termux-unboost — desactiva wake-lock"
            _perf_ok "alias ollama-fast — lanza ollama con nice -n -10"
            _perf_ok "alias mem-check — muestra RAM libre rápido"
            echo ""
            _perf_warn "Ejecuta: source ~/.bashrc  para activar en sesión actual"
            echo ""
          ...
          3|a|A)
            ...
            if ! _has_termux_api; then
              ...
              pkg install -y termux-api 2>/dev/null \
                ...
                || { _perf_warn "No se pudo instalar — instala Termux:API desde F-Droid"; read -r _ < /dev/tty; continue; }
            fi
            if _wakelock_active; then
              ...
              _wakelock_active \
                ...
                || _perf_warn "Wake-lock iniciado en background (verifica con: pgrep termux-wake-lock)"
            fi
          ...
          4|a|A)
            ...
            # a) Script de boot — Termux:Boot lo ejecuta al arrancar Android
...
#!/data/data/com.termux/files/usr/bin/bash
# Auto-generado por termux-ai-stack menu.sh [p]
# Se ejecuta al arrancar Android via Termux:Boot

# Wake-lock inmediato para evitar throttling
termux-wake-lock 2>/dev/null &

# Esperar a que el sistema arranque completamente
sleep 10

# Notificación de que Termux arrancó
termux-notification \
...
# Si n8n estaba corriendo, reiniciarlo
[ -f "$HOME/start_servidor.sh" ] && \
  tmux new-session -d -s "n8n-server" "bash $HOME/start_servidor.sh" 2>/dev/null

# Si ollama estaba corriendo, reiniciarlo
[ -f "$HOME/ollama_start.sh" ] && \
  tmux new-session -d -s "ollama-server" "bash $HOME/ollama_start.sh" 2>/dev/null
BOOT_EOF
            chmod +x "$BOOT_SCRIPT"
            _perf_ok "Boot script: $BOOT_SCRIPT"

            # b) Notificación persistente (mantiene Termux en foreground)
            if _has_termux_api; then
              termux-notification \
                --title "Termux AI Stack" \
                --content "Stack activo — keepalive ON" \
                --id 1001 \
                --ongoing 2>/dev/null \
                && _perf_ok "Notificación persistente activa (Termux en foreground)" \
            ...
            echo ""
            _perf_warn "Para auto-inicio al reiniciar Android: instala Termux:Boot desde F-Droid"
            echo -e "  ${DIM}F-Droid → buscar 'Termux:Boot' → instalar → reiniciar Android${NC}"
            echo ""
            [ "$POPT" = "4" ] && { read -r _ < /dev/tty; continue; }
            ;&

          # ── [5] Parchear scripts de inicio ────────────────────
          5|a|A)
            clear; echo ""
            ...
            _patch_wakelock() {
              ...
              local label="$2"
              if [ ! -f "$script" ]; then
                _perf_warn "$label: $script no encontrado — omitiendo"
                return
              fi
              if grep -q "termux-wake-lock" "$script" 2>/dev/null; then
                _perf_ok "$label: ya tiene wake-lock — sin cambios"
                return
              fi
              ...
              chmod +x "$script"
              _perf_ok "$label: wake-lock inyectado en $script"
            }
        ...
        # Si llegamos aquí desde [a] (fall-through completo)
        ...
        # Refrescar estado wake-lock en el header
        WL_STATUS="$(_wakelock_active && echo "${GREEN}● activo${NC}" || echo "${YELLOW}○ inactivo${NC}")"
        API_STATUS="$(_has_termux_api && echo "${GREEN}✓${NC}" || echo "${YELLOW}✗ instalar pkg termux-api${NC}")"
      done ;;
    s|S|q|Q|"")
