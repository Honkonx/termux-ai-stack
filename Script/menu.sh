...
install_module() {
  ...
  if [ "$module_key" = "claude" ]; then
    echo -e "  ║  ${NC}[1] npm directo (Opción C — recomendado)${CYAN}${BOLD} ║"
    echo -e "  ║  ${GREEN}    ✓ descarga desde npmjs.com${CYAN}${BOLD}          ║"
    ...
    case "$OPT" in
      ...
      7) # Iniciar Dashboard
        ...
        if pgrep -f "dashboard_server.py" &>/dev/null; then
          ...
          echo "  Instala desde el menú principal [6] → Instalar"
        fi
        echo ""; read -r _ < /dev/tty ;;
...
while true; do
  ...
  draw_module "6" "⬡" "Remote"       "$RM_STATE"  "${RM_VER}  ${RM_EXTRA}" "$RM_CMD"
