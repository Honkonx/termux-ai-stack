...
_ollama_config_sql() {
  ...
  while true; do
    clear; echo ""
    echo -e "  ${CYAN}${BOLD}⚙ Config historial SQLite${NC}"
    echo -e "  ${DIM}──────────────────────────────${NC}"
    echo -e "  ${BOLD}Contexto activo (RAM)${NC}  — actual: ${GREEN}${OL_RAM_MSGS} msg${NC}"
    echo -e "  ${DIM}  Más = mejor memoria, más lento${NC}"
    echo -e "  [2] 2 msg ~50MB   [4] 4 msg ~100MB  ← recomendado"
    echo -e "  [6] 6 msg ~150MB  [8] 8 msg ~200MB"
    echo -e "  [c] Personalizado (1-20)"
    echo ""
    echo -e "  ${BOLD}Historial en disco${NC}      — actual: ${CYAN}${OL_DISK_MSGS} msg${NC}"
    echo -e "  [d] 20   [e] 50 ← rec   [f] 100   [g] sin límite"
    echo -e "  [h] Personalizado"
    echo ""
    echo -e "  [i] Ver estadísticas BD   [j] Borrar toda la BD"
    echo -e "  ${DIM}──────────────────────────────${NC}"
    echo -e "  [b] Volver"
    echo ""
    printf "  Opción: "
    read -r COPT < /dev/tty
    case "$COPT" in
      2) OL_RAM_MSGS=2;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 2 msg (~50MB)" ;;
      4) OL_RAM_MSGS=4;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 4 msg (~100MB)" ;;
      6) OL_RAM_MSGS=6;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 6 msg (~150MB)" ;;
      8) OL_RAM_MSGS=8;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: 8 msg (~200MB)" ;;
      c|C)
        printf "  Mensajes en contexto (1-20): "
        read -r VAL < /dev/tty
        [[ "$VAL" =~ ^[0-9]+$ ]] && [ "$VAL" -ge 1 ] && [ "$VAL" -le 20 ]           && { OL_RAM_MSGS=$VAL; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Contexto: $VAL msg"; }           || echo -e "  ${RED}[ERROR]${NC} Valor inválido (1-20)" ;;
      d|D) OL_DISK_MSGS=20;   _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 20 msg" ;;
      e|E) OL_DISK_MSGS=50;   _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 50 msg" ;;
      f|F) OL_DISK_MSGS=100;  _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: 100 msg" ;;
      g|G) OL_DISK_MSGS=9999; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: sin límite" ;;
      h|H)
        printf "  Mensajes en disco (10-9999): "
        read -r VAL < /dev/tty
        [[ "$VAL" =~ ^[0-9]+$ ]] && [ "$VAL" -ge 10 ]           && { OL_DISK_MSGS=$VAL; _ollama_save_cfg; echo -e "  ${GREEN}[OK]${NC} Disco: $VAL msg"; }           || echo -e "  ${RED}[ERROR]${NC} Valor inválido" ;;
      i|I)
...
conn.close()
print(f'  Mensajes  : {total}  |  Chats: {chats}  |  Imágenes: {imgs}')
...
submenu_ollama() {
  ...
  while true; do
    ...
    echo ""
    echo -e "  ${BOLD}[2]${NC} Chat rápido        ${DIM}sin historial${NC}"
    echo -e "  ${BOLD}[3]${NC} Chat completo       ${DIM}SQLite · ${OL_RAM_MSGS}msg RAM · ${OL_DISK_MSGS} disco${NC}"
    echo -e "  ${BOLD}[4]${NC} Chat con imágenes   ${DIM}/imagen RUTA · /camara${NC}"
    echo -e "  ${DIM}──────────────────────────────${NC}"
    ...
    case "$OPT" in
      ...
      1)
        clear; echo ""
        if [ "$state" = "running" ]; then
          printf "  Ollama activo. ¿Detener? (s/n): "
          read -r _ST < /dev/tty
          [ "$_ST" = "s" ] || [ "$_ST" = "S" ] && {
      ...
      2)
        ...
        MODEL="$PICKED_MODEL"
        echo ""
        echo -e "  ${GREEN}[OK]${NC} Chat rápido — ${CYAN}$MODEL${NC}"
        echo -e "  ${DIM}Escribe tu mensaje. /bye para salir${NC}"; echo ""
      ...
      4)
        clear; echo ""
        _ollama_ensure_server || { read -r _ < /dev/tty; continue; }
        echo -e "  ${CYAN}Modelos de visión:${NC}"
        _ollama_pick_model "vision"
        if [ -z "$PICKED_MODEL" ]; then
          echo ""
          echo -e "  ${BOLD}¿Descargar modelo de visión?${NC}"
      ...
      # ── [5] Modelos — pantalla plana sin submenú anidado ────
      5)
        while true; do
          clear; echo ""
          echo -e "  ${CYAN}${BOLD}◎ Modelos instalados${NC}"
          echo -e "  ${DIM}──────────────────────────────${NC}"
          ollama list 2>/dev/null | tail -n +2 | \
            awk '{printf "  %-28s  %s %s\n", $1, $3, $4}' | head -15
          echo ""
          echo -e "  ${DIM}──────────────────────────────${NC}"
          echo -e "  ${BOLD}[d]${NC} Descargar   ${BOLD}[r]${NC} Eliminar   ${BOLD}[b]${NC} Volver"
          echo ""; printf "  Opción: "
          read -r MOPT < /dev/tty
          case "$MOPT" in
            d|D)
              ...
              echo ""
              echo -e "  ${BOLD}VISIÓN${NC}"
              echo "    [f] moondream:1.8b  ~1.1GB  rápido"
              echo "    [g] llava-phi3:3.8b ~2.5GB  mejor calidad ← rec"
              echo "    [h] llava:7b        ~4.7GB  alta calidad"
              echo "    [m] Escribir nombre manualmente"
              echo ""; printf "  Elige: "
              read -r DC < /dev/tty
              case "$DC" in
                ...
                m|M) printf "  Nombre del modelo: "; read -r DL < /dev/tty ;;
                *) DL="" ;;
              esac
            ...
            r|R)
              ...
              _ollama_pick_model
              [ -n "$PICKED_MODEL" ] && {
                printf "  ¿Eliminar %s? (s/n): " "$PICKED_MODEL"
                read -r _DEL < /dev/tty
                [ "$_DEL" = "s" ] || [ "$_DEL" = "S" ] && {
