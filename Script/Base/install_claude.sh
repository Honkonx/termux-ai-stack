...
if check_done "nodejs"; then
  ...
  if command -v node &>/dev/null; then
    ...
    if [ "$NODE_VER" -ge 25 ] 2>/dev/null; then
      # Node v25+ es INCOMPATIBLE con @anthropic-ai/claude-code
      # npm instala el wrapper pero cli.js queda en 0 bytes
      # Solución: reemplazar por nodejs-lts (v22)
      warn "Node.js $(node --version) detectado — v25+ es incompatible con Claude Code"
      warn "Reemplazando por nodejs-lts (v22 LTS — versión compatible)..."
      pkg uninstall nodejs -y 2>/dev/null || true
      pkg install nodejs-lts -y \
        -o Dpkg::Options::="--force-confdef" \
        -o Dpkg::Options::="--force-confold" || \
        error "Error instalando nodejs-lts. Verifica conexión."
      log "Node.js reemplazado: $(node --version)"
    elif [ "$NODE_VER" -ge 18 ] 2>/dev/null; then
      log "Node.js $(node --version) ✓ (compatible con Claude Code)"
    else
      warn "Node.js $(node --version) — versión < 18, actualizando a LTS..."
      pkg install nodejs-lts -y \
        -o Dpkg::Options::="--force-confdef" \
        -o Dpkg::Options::="--force-confold"
    ...
    info "Node.js no encontrado — instalando nodejs-lts (v22 LTS)..."
    pkg install nodejs-lts -y \
      -o Dpkg::Options::="--force-confdef" \
      -o Dpkg::Options::="--force-confold" || \
      error "Error instalando nodejs-lts. Verifica conexión."
...
if check_done "claude_npm"; then
  ...
  # Versión confirmada funcional en Termux ARM64 (Bionic libc)
  # Las versiones "latest" usan binario nativo que no corre en Bionic
  ...
  # Versiones recientes de @anthropic-ai/claude-code requieren
  # un binario nativo (linux-arm64 con glibc) que no existe en
  # Termux (usa Bionic). Intentamos latest primero y hacemos
  # fallback a la última versión que incluía cli.js funcional.
  # ─────────────────────────────────────────────────────────────

  CLAUDE_OK=false

  # Intento 1: latest con postinstall normal
  info "Intento 1/2: instalando versión latest..."
  ...
  # Intento 2: versión específica conocida como funcional en Termux
  if [ "$CLAUDE_OK" = "false" ]; then
    warn "Latest no incluye cli.js compatible — usando v${CLAUDE_VERSION_FALLBACK} (confirmada en Termux ARM64)..."
    npm uninstall -g @anthropic-ai/claude-code 2>/dev/null || true
    npm cache clean --force 2>/dev/null || true
    echo ""
  ...
  if [ "$CLAUDE_OK" = "false" ]; then
    ...
    echo ""
    echo "  Prueba manualmente (con buena conexión WiFi o datos):"
...
ALIASES
  ...
  log "Alias 'claude' → node ${CLI_FINAL}"
  log "Alias 'claude-update' configurado"
