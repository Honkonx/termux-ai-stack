...
if check_done "claude_npm"; then
  ...
  # ── Validación de cli.js ─────────────────────────────────────
  # Un cli.js válido para Node debe comenzar con código JS, no con shebang bash.
  # Las versiones latest (v2.2+) reemplazan cli.js con un wrapper bash que
  # ejecuta un binario nativo glibc — incompatible con Termux (Bionic libc).
  # Detectamos esto verificando que node pueda ejecutar el archivo sin SyntaxError.
  is_valid_cli() {
    local path="$1"
    [ -f "$path" ] && [ -s "$path" ] || return 1
    node --input-type=module < /dev/null 2>/dev/null  # warm node
    node "$path" --version 2>&1 | grep -qv "SyntaxError"
  }

  # Intento 1: latest — verifica si cli.js es JS válido (no wrapper bash)
  info "Intento 1/2: probando versión latest..."
  npm install -g @anthropic-ai/claude-code 2>&1 | tail -3
  echo ""

  CLI_PATH_NEW=$(find_cli)
  if is_valid_cli "$CLI_PATH_NEW"; then
    CLAUDE_OK=true
    log "Versión latest instalada con cli.js válido ✓"
  else
    warn "Latest instaló cli.js no compatible (wrapper bash/binario nativo glibc)"
  fi

  # Intento 2: fallback a v2.1.111 (última con cli.js puro JS en Termux)
  if [ "$CLAUDE_OK" = "false" ]; then
    warn "Usando v${CLAUDE_VERSION_FALLBACK} — confirmada en Termux ARM64 + Node v24..."
    npm uninstall -g @anthropic-ai/claude-code 2>/dev/null || true
    npm cache clean --force 2>/dev/null || true
    echo ""
    ...
    if is_valid_cli "$CLI_PATH_NEW"; then
      CLAUDE_OK=true
      log "v${CLAUDE_VERSION_FALLBACK} instalada con cli.js válido ✓"
    fi
  fi
