...
# ── Helper: asegurar restore.sh disponible ───────────────────
_ensure_restore_sh() {
...
# ── Helper: asegurar script de instalación disponible ────────
...
# ── Instalar módulo — menú de opciones ───────────────────────
install_module() {
  local name="$1"
  local module_key="$2"
  ...
  # n8n tiene menú extendido de 4 opciones por su dependencia del rootfs
  if [ "$module_key" = "n8n" ]; then
    echo -e "  ║  ${NC}[1] Todo desde GitHub${CYAN}${BOLD}                   ║"
    echo -e "  ║     ${GRAY}rootfs + n8n precompilados · recomendado${CYAN}${BOLD}║"
    echo -e "  ║  ${NC}[2] Todo limpio${CYAN}${BOLD}                         ║"
    echo -e "  ║     ${GRAY}proot-distro + npm install · 25-40 min${CYAN}${BOLD} ║"
    echo -e "  ║  ${NC}[3] Rootfs GitHub + n8n limpio${CYAN}${BOLD}           ║"
    echo -e "  ║     ${GRAY}rootfs de GitHub, n8n fresco con npm${CYAN}${BOLD}   ║"
    echo -e "  ║  ${NC}[4] Rootfs limpio + n8n GitHub${CYAN}${BOLD}           ║"
    echo -e "  ║     ${GRAY}proot-distro + n8n del paquete GitHub${CYAN}${BOLD}  ║"
  else
  ...
  # ── Opciones exclusivas de n8n ───────────────────────────────
  if [ "$module_key" = "n8n" ]; then
    case "$INST_OPT" in
      1)
        # Todo desde GitHub: proot (part6) + n8n (part5)
        _ensure_restore_sh || return 1
        bash "$HOME/restore.sh" --module proot --source github < /dev/tty
        bash "$HOME/restore.sh" --module n8n   --source github < /dev/tty
        echo ""
        read -r _ < /dev/tty
        return 0
        ;;
      2)
        # Todo limpio: pasar a install_n8n.sh con modo limpio
        _ensure_install_script "$script" || return 1
        N8N_INSTALL_MODE=2 bash "$dest" < /dev/tty
        echo ""
        read -r _ < /dev/tty
        return 0
        ;;
      3)
        # Rootfs GitHub + n8n limpio
        _ensure_restore_sh || return 1
        bash "$HOME/restore.sh" --module proot --source github < /dev/tty
        _ensure_install_script "$script" || return 1
        N8N_INSTALL_MODE=3 bash "$dest" < /dev/tty
        echo ""
        read -r _ < /dev/tty
        return 0
        ;;
      4)
        # Rootfs limpio + n8n GitHub
        _ensure_restore_sh || return 1
        _ensure_install_script "$script" || return 1
        N8N_INSTALL_MODE=4 bash "$dest" < /dev/tty
        echo ""
        read -r _ < /dev/tty
        return 0
        ;;
  ...
  # ── Opciones genéricas (Claude, Ollama, Expo) ─────────────────
  case "$INST_OPT" in
    2)
      _ensure_restore_sh || return 1
