...
if pgrep -x sshd &>/dev/null; then
  ...
  echo "  Conectar: ssh -p 8022 $(whoami)@${IP:-<tu_IP>}"
  exit 0
fi
...
alias ssh-info='IP=$(ip addr show wlan0 2>/dev/null | grep "inet " | awk "{print \$2}" | cut -d"/" -f1); echo "Conectar: ssh -p 8022 $(whoami)@${IP:-<tu_IP>}"'
