...
// v1.7.0 | Mayo 2026
// Fix chat connErr · Submenú n8n completo (logs, token, webhook)
...
export default function App() {
  ...
  const [n8nLogsLoading,setN8nLogsLoading]= useState(false);
  const [n8nCfToken,    setN8nCfToken]    = useState('');
  const [n8nCfMode,     setN8nCfMode]     = useState(null); // null|'fija'|'temporal'
  const [n8nTokenSaving,setN8nTokenSaving]= useState(false);
  const [n8nWebhook,    setN8nWebhook]    = useState('');
  const [n8nWebhookSaving,setN8nWebhookSaving] = useState(false);
  const [n8nWebhookSaved, setN8nWebhookSaved]  = useState(false);
  ...
  const fetchStatus = useCallback(async (manual = false) => {
    ...
    try {
      ...
      // No marcar connErr si hay un chat en progreso — evita falso "Sin conexión"
      setChatLoading(prev => { if (!prev) setConnErr(true); return prev; });
    } finally {
  ...
  const fetchN8nInfo = useCallback(async () => {
    try {
      const r = await apiFetch('/api/n8n/info');
      if (r.ok) {
        const d = await r.json();
        setN8nUrl(d.url || '');
        setN8nCfMode(d.cf_mode || 'temporal');
        setN8nWebhook(d.webhook_url || '');
      }
    } catch {}
  }, []);

  const fetchN8nLogs = useCallback(async () => {
    setN8nLogsLoading(true);
    try {
      const r = await apiFetch('/api/n8n/logs', {}, 8000);
      if (r.ok) { const d = await r.json(); setN8nLogs(d.logs || 'Sin logs disponibles'); }
    } catch { setN8nLogs('[Error obteniendo logs — n8n corriendo?]'); }
    finally { setN8nLogsLoading(false); }
  }, []);

  const saveN8nToken = useCallback(async (token, remove = false) => {
    setN8nTokenSaving(true);
    try {
      const r = await apiFetch('/api/n8n/token', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: remove ? '' : token, remove }),
      });
      const d = await r.json();
      if (d.ok) {
        setN8nCfMode(remove ? 'temporal' : 'fija');
        setN8nCfToken('');
        Alert.alert('✓', d.msg);
      } else { Alert.alert('Error', d.msg || 'No se pudo guardar'); }
    } catch { Alert.alert('Error', 'Sin conexión'); }
    finally { setN8nTokenSaving(false); }
  }, []);

  const saveN8nWebhook = useCallback(async () => {
    if (!n8nWebhook.trim()) return;
    setN8nWebhookSaving(true);
    try {
      const r = await apiFetch('/api/n8n/webhook', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ webhook_url: n8nWebhook.trim() }),
      });
      const d = await r.json();
      if (d.ok) { setN8nWebhookSaved(true); setTimeout(() => setN8nWebhookSaved(false), 2500); }
    } catch {} finally { setN8nWebhookSaving(false); }
  }, [n8nWebhook]);
    ...
    if (m.id === 'n8n') return (
      <View style={S.expanded}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          <View style={[P.base, n8nCfMode === 'fija'
            ? { backgroundColor: C.greenDim, borderColor: C.green + '66' }
            : { backgroundColor: C.yellowDim, borderColor: C.yellow + '66' }]}>
            <View style={[P.dot, { backgroundColor: n8nCfMode === 'fija' ? C.green : C.yellow }]} />
            <Text style={[P.text, { color: n8nCfMode === 'fija' ? C.green : C.yellow }]}>
              {n8nCfMode === 'fija' ? 'URL fija' : 'URL temporal'}
            </Text>
          </View>
        </View>
        {n8nUrl ? (
          ...
          <Text style={S.expHint}>{m.running ? 'Tunnel iniciando... toca ↻' : 'Inicia n8n para obtener URL'}</Text>
        )}
        <TouchableOpacity style={S.linkBtn} onPress={fetchN8nInfo}>
        ...
        {/* Token cloudflared */}
        <SectionLabel label="TOKEN CLOUDFLARED" />
        <Text style={S.expHint}>
          {n8nCfMode === 'fija'
            ? '● Token configurado — URL fija activa'
            : '○ Sin token — URL cambia en cada reinicio'}
        </Text>
        <Text style={[S.inputLabel, { marginTop: 10 }]}>
          {n8nCfMode === 'fija' ? 'Reemplazar token' : 'Nuevo token (Cloudflare Zero Trust)'}
        </Text>
        <TextInput
          style={S.input}
          value={n8nCfToken}
          onChangeText={setN8nCfToken}
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <ActionBtn
            label={n8nTokenSaving ? 'Guardando...' : 'Guardar token'}
            onPress={() => {
              if (!n8nCfToken.trim()) { Alert.alert('Token vacío', 'Ingresa el token primero'); return; }
              Alert.alert('Guardar token', '¿Activar URL fija con este token?', [
                { text: 'Cancelar', style: 'cancel' },
                { text: 'Guardar', onPress: () => saveN8nToken(n8nCfToken) },
              ]);
            }}
            loading={n8nTokenSaving}
          />
          {n8nCfMode === 'fija' && (
            <ActionBtn
              label="Quitar token"
              danger
              onPress={() => Alert.alert('Quitar token', '¿Volver a URL temporal?', [
                { text: 'Cancelar', style: 'cancel' },
                { text: 'Quitar', style: 'destructive', onPress: () => saveN8nToken('', true) },
              ])}
            />
          )}
        </View>
        ...
        {/* Webhook URL */}
        <SectionLabel label="WEBHOOK URL (N8N_WEBHOOK_URL)" />
        <Text style={S.expHint}>URL fija de tu dominio para que Telegram y servicios externos apunten aquí.</Text>
        <TextInput
          style={[S.input, { marginTop: 8 }]}
          value={n8nWebhook}
          onChangeText={setN8nWebhook}
          placeholder="https://bot.tudominio.shop"
          placeholderTextColor={C.dim}
          autoCapitalize="none"
          autoCorrect={false}
          keyboardType="url"
        />
        <ActionBtn
          label={n8nWebhookSaved ? '✓ Guardado' : n8nWebhookSaving ? 'Guardando...' : 'Guardar webhook URL'}
          onPress={saveN8nWebhook}
          loading={n8nWebhookSaving}
          color={n8nWebhookSaved ? C.green : C.cyan}
        />
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <SectionLabel label="LOGS n8n (últimas líneas)" />
          <TouchableOpacity onPress={fetchN8nLogs} disabled={n8nLogsLoading}>
            {n8nLogsLoading
        ...
        {n8nLogs ? (
          ...
          >
            <Text style={S.logsText} selectable>{n8nLogs}</Text>
          </ScrollView>
        ) : (
          <TouchableOpacity style={S.logsEmpty} onPress={fetchN8nLogs}>
            <Text style={{ color: C.dim, fontSize: 12 }}>Toca ↻ para cargar logs</Text>
          </TouchableOpacity>
        )}
        ...
        <InfoRow k="Webhook URL"    v={n8nWebhook || n8nUrl || '—'} vc={C.cyan} />
...
const S = StyleSheet.create({
  ...
  logsBox:   { backgroundColor: C.bg, borderRadius: 8, borderWidth: 1, borderColor: C.border, padding: 10, maxHeight: 200, marginBottom: 4 },
  logsText:  { fontSize: 10, color: C.green, fontFamily: Platform.OS === 'android' ? 'monospace' : 'Courier', lineHeight: 16 },
  logsEmpty: { backgroundColor: C.bg, borderRadius: 8, borderWidth: 1, borderColor: C.border, padding: 14, alignItems: 'center', marginBottom: 4 },
